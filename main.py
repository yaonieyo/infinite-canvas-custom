...
GITHUB_REPO_URL = "https://github.com/yaonieyo/infinite-canvas-custom"
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/yaonieyo/infinite-canvas-custom/main/VERSION"
GITHUB_TREE_URL = "https://api.github.com/repos/yaonieyo/infinite-canvas-custom/git/trees/main?recursive=1"
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/yaonieyo/infinite-canvas-custom/main"
...
SUPPORTED_PROVIDER_PROTOCOLS = {"openai", "apimart", "gemini", "gemini-cli", "volcengine", "runninghub", "jimeng", "codex", "doubao-pool"}
...
APIMART_DEFAULT_VIDEO_MODELS = [
    # APIMart official model IDs; display names do not always match API IDs.
...
def provider_env_key_value(provider_id: str) -> str:
    ...
    if provider_id == "doubao-video":
        return "local-doubao-key"
    return ""
...
def default_api_providers():
    ...
    return [
        ...
        },
        {
            ...
            "image_models": [],
            "chat_models": [],
            "video_models": ["seedance_2_0_mini", "seedance_2_0_fast"],
            "ms_loras": [],
            "ms_defaults_version": 0,
        },
    ]
...
def merge_default_api_providers(providers, inject_missing=True):
    ...
    doubao_video_default = next((d for d in default_api_providers() if d["id"] == "doubao-video"), None)
    if doubao_video_default:
        current = next((item for item in merged if item.get("id") == "doubao-video"), None)
        if not current:
            merged.append(doubao_video_default)
        else:
            current["protocol"] = "doubao-pool"
            if not current.get("base_url"):
                current["base_url"] = doubao_video_default["base_url"]
            current["image_models"] = []
            current["chat_models"] = []
            current["video_models"] = model_list_from_values(current.get("video_models") or doubao_video_default["video_models"])
...
def normalize_provider(item):
    ...
    if provider_id == "jimeng" or protocol == "jimeng":
        protocol = "jimeng"
        base_url = ""
    if provider_id == "doubao-video" or protocol == "doubao-pool":
        protocol = "doubao-pool"
        base_url = base_url or "http://127.0.0.1:17888"
    ...
    for dirname in children:
        ...
        try:
            ...
            records.append({
                ...
                "surfaces": [str(item).strip() for item in (raw.get("surfaces") or []) if str(item).strip()][:10],
...
OPTIMIZE_PROMPT_SYSTEM = (
    "你是一位专业的 AI 图像生成提示词工程师。请优化以下提示词，"
    "使其在 AI 图像生成中产生更高质量、更详细的画面效果。\n\n"
    "规则：\n"
    "1. 保持原始意图、风格和主题完全不变\n"
    "2. 补充关于光照、构图、色彩、材质、情绪的具体细节\n"
    "3. 使用生动、具象的描述性语言\n"
    "4. 不要添加任何解释、前缀或后缀——只输出优化后的提示词原文\n"
    "5. 保持简洁但细节丰富\n"
    "6. 如果原提示词是中文，用中文输出；如果是英文，用英文输出"
)
...
# 协议固定、不支持单模型覆盖的内置平台
FIXED_PROTOCOL_PROVIDER_IDS = {"modelscope", "volcengine", "jimeng", "runninghub", "doubao-video"}
...
def is_doubao_pool_provider(provider):
    return provider_protocol(provider) == "doubao-pool" or str((provider or {}).get("id") or "").strip().lower() == "doubao-video"
    ...
    try:
        ...
        if ";" in server:
            ...
            for item in server.split(";"):
                if "=" in item:
                    proxy_type, value = item.split("=", 1)
                    parts[proxy_type.strip().lower()] = value.strip()
            server = parts.get("https") or parts.get("http") or next((value for value in parts.values() if value), "")
        if not server:
            return ""
        if not re.match(r"^[a-z]+://", server, re.I):
...
def codex_subprocess_env():
    env = os.environ.copy()
    proxy = (
        codex_env_value("HTTPS_PROXY")
        or codex_env_value("HTTP_PROXY")
        or codex_env_value("ALL_PROXY")
        or windows_system_proxy_url()
    )
    if proxy:
...
def gpt_image_2_skill_command():
    ...
    if configured:
        return [configured]
    candidates = []
    node_dir = os.path.join(os.path.expanduser("~"), ".codex", "tools", "node")
    candidates.append((
        os.path.join(node_dir, "node.exe" if os.name == "nt" else "node"),
        os.path.join(node_dir, "node_modules", "gpt-image-2-skill", "bin", "gpt-image-2-skill.js"),
    ))
    shim = shutil.which("gpt-image-2-skill.cmd") or shutil.which("gpt-image-2-skill")
    if shim:
        basedir = os.path.dirname(shim)
        candidates.append((
            os.path.join(basedir, "node.exe" if os.name == "nt" else "node"),
            os.path.join(basedir, "node_modules", "gpt-image-2-skill", "bin", "gpt-image-2-skill.js"),
        ))
    for node_path, script_path in candidates:
        if os.path.isfile(node_path) and os.path.isfile(script_path):
            return [node_path, script_path]
    if os.name == "nt":
        exe = shutil.which("gpt-image-2-skill.exe") or shutil.which("gpt-image-2-skill.cmd") or shutil.which("gpt-image-2-skill.ps1") or shutil.which("gpt-image-2-skill")
    else:
        exe = shutil.which("gpt-image-2-skill") or shutil.which("gpt-image-2-skill.exe") or shutil.which("gpt-image-2-skill.cmd")
...
def codex_postprocess_image_to_requested_size(path="", requested_size="", provider=""):
    ...
    try:
        ...
        label = "Gemini CLI" if provider_text == "gemini-cli" else ("Codex GPT Image 2" if provider_text == "codex" else "生图")
        print(f"{label} 图片尺寸后处理失败：{exc}")
        return ""

def postprocess_output_url_to_requested_size(url="", requested_size="", provider=""):
    path = output_file_from_url(url)
    if not path:
        return url
    processed_path = codex_postprocess_image_to_requested_size(path, requested_size, provider)
    if not processed_path:
        return url
    return codex_output_url_from_path(processed_path) or url

async def generate_codex_provider_image_via_gpt_image_2_skill(prompt, size, model, ref_paths=None):
    command_prefix = gpt_image_2_skill_command()
...
DOUBAO_POOL_DEFAULT_BASE_URL = "http://127.0.0.1:17888"
DOUBAO_POOL_DEFAULT_KEY = "local-doubao-key"
DOUBAO_POOL_VIDEO_MODELS = ["seedance_2_0_mini", "seedance_2_0_fast"]

def doubao_pool_base_url(provider):
    return str((provider or {}).get("base_url") or DOUBAO_POOL_DEFAULT_BASE_URL).strip().rstrip("/") or DOUBAO_POOL_DEFAULT_BASE_URL

def doubao_pool_api_key(provider):
    return str(provider_env_key_value((provider or {}).get("id") or "doubao-video") or DOUBAO_POOL_DEFAULT_KEY).strip()

def doubao_pool_headers(provider):
    headers = {"Accept": "application/json", "Content-Type": "application/json"}
    token = doubao_pool_api_key(provider)
    if token:
        headers["Authorization"] = bearer_auth_value(token)
    return headers

def doubao_pool_model(value):
    text = str(value or "").strip().lower().replace("-", "_")
    if "fast" in text:
        return "seedance_2_0_fast"
    if "mini" in text or "2_0" in text or "2.0" in text:
        return "seedance_2_0_mini"
    return "seedance_2_0_mini"

def save_local_video_path_to_output(path_value, prefix="doubao_video_"):
    path_text = str(path_value or "").strip()
    if not path_text:
        return ""
    existing = output_file_from_url(path_text)
    if existing and os.path.isfile(existing):
        path_text = existing
    if path_text.startswith("/assets/") or path_text.startswith("/output/"):
        return path_text
    if not os.path.isfile(path_text):
        return path_text
    ext = os.path.splitext(path_text)[1].lower()
    if ext not in {".mp4", ".webm", ".mov", ".m4v", ".avi", ".mkv", ".flv"}:
        ext = ".mp4"
    filename = f"{prefix}{uuid.uuid4().hex[:10]}{ext}"
    dst = output_path_for(filename, "output")
    os.makedirs(os.path.dirname(dst), exist_ok=True)
    shutil.copy2(path_text, dst)
    return output_url_for(filename, "output")

async def doubao_pool_reference_path(ref):
    ...
    if url.startswith("data:image/") and ";base64," in url:
        ...
        if ext == ".jpe":
            ext = ".jpg"
        filename = f"doubao_ref_{uuid.uuid4().hex[:10]}{ext}"
        dst = output_path_for(filename, "input")
        os.makedirs(os.path.dirname(dst), exist_ok=True)
        with open(dst, "wb") as f:
            f.write(base64.b64decode(encoded, validate=False))
        return os.path.abspath(dst)
    if url.startswith(("http://", "https://")):
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(connect=15.0, read=60.0, write=20.0, pool=10.0), follow_redirects=True, trust_env=False) as client:
                response = await client.get(url)
                response.raise_for_status()
                content_type = (response.headers.get("Content-Type") or "").split(";", 1)[0].lower()
                ext = mimetypes.guess_extension(content_type) or os.path.splitext(urllib.parse.urlparse(url).path)[1] or ".jpg"
                if ext == ".jpe":
                    ext = ".jpg"
                filename = f"doubao_ref_{uuid.uuid4().hex[:10]}{ext}"
                dst = output_path_for(filename, "input")
...
async def doubao_pool_reference_paths(payload):
    paths = []
    for ref in (payload.images or [])[:10]:
        path_value = await doubao_pool_reference_path(ref)
        if path_value:
            paths.append(path_value)
    return paths

def is_doubao_pool_playable_video_value(value):
    text = str(value or "").strip()
    if not text:
        return False
    if "平台暂不支持" in text or "暂不支持" in text:
        return False
    video_exts = {".mp4", ".webm", ".mov", ".m4v", ".avi", ".mkv", ".flv"}
    local_path = output_file_from_url(text)
    if local_path:
        return os.path.splitext(local_path)[1].lower() in video_exts
    if os.path.isfile(text):
        return os.path.splitext(text)[1].lower() in video_exts
    parsed = urllib.parse.urlparse(text)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        return False
    ...
    query_values = " ".join(v for values in query.values() for v in values).lower()
    if "video_mp4" in query_values or "video/" in query_values:
        return True
    filename = " ".join(query.get("filename", [])).lower()
    if any(filename.endswith(ext) for ext in video_exts):
        return True
    if "videoweb-download" in parsed.netloc.lower():
        return True
    return False

def doubao_pool_result_values(raw):
    values = []
    def add_value(value):
        text = str(value or "").strip()
        if text and is_doubao_pool_playable_video_value(text):
            values.append(text)

    def add_from_container(value):
        if not value:
            return
        if isinstance(value, str):
            add_value(value)
        ...
        if isinstance(value, dict):
            ...
            for key in ("data", "result", "output", "outputs", "videos", "items"):
                add_from_container(value.get(key))
    add_from_container(raw)
    seen = set()
    out = []
    for item in values:
        if item in seen:
...
async def normalize_doubao_pool_video_output(value):
    text = str(value or "").strip()
    if not text:
        return ""
    if not is_doubao_pool_playable_video_value(text):
        return ""
    if text.startswith(("http://", "https://", "/assets/", "/output/")):
        saved = await save_remote_video_to_output(text, prefix="doubao_video_")
        return saved if saved and saved != text else ""
    return save_local_video_path_to_output(text, prefix="doubao_video_")

async def generate_doubao_pool_video(payload: CanvasVideoRequest, provider):
    base_url = doubao_pool_base_url(provider)
    refs = await doubao_pool_reference_paths(payload)
    model = doubao_pool_model(payload.model)
    body = {
        "source": "infinite-canvas",
        "prompt": str(payload.prompt or "").strip(),
        "model": model,
        "referenceImagePath": refs[0] if refs else "",
        "referenceImagePaths": refs,
        "removeWatermark": True,
        "duration": int(payload.duration or 5),
        "aspectRatio": str(payload.aspect_ratio or "16:9"),
        "resolution": str(payload.resolution or ""),
    }
    timeout = httpx.Timeout(connect=10.0, read=30.0, write=60.0, pool=10.0)
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, trust_env=False) as client:
        try:
            response = await client.post(f"{base_url}/api/generate", headers=doubao_pool_headers(provider), json=body)
        except httpx.RequestError as exc:
            raise HTTPException(status_code=502, detail=f"无法连接豆包账号池接口服务：{base_url}。请先打开 /Applications/豆包账号池接口服务.app，并确认本地 API 已开启。") from exc
        if response.status_code == 401:
            raise HTTPException(status_code=401, detail="豆包账号池接口服务 API Key 不正确。请在画布 API 设置里给“豆包视频接口”填写和账号池服务一致的 API Key。")
        if response.status_code >= 400:
            raise HTTPException(status_code=response.status_code, detail=f"豆包账号池接口服务返回错误：{response.text[:800]}")
        try:
            created = response.json()
        except Exception as exc:
            raise HTTPException(status_code=502, detail=f"豆包账号池接口服务返回非 JSON：{response.text[:500]}") from exc

    request_id = str((created or {}).get("requestId") or (created or {}).get("request_id") or "").strip()
    if not request_id:
        urls = doubao_pool_result_values(created)
        if urls:
            local_urls = [await normalize_doubao_pool_video_output(url) for url in urls]
            videos = [u for u in local_urls if u]
            if not videos:
                raise HTTPException(status_code=502, detail=f"豆包账号池返回了视频链接，但画布下载保存失败：{urls[0]}")
            return {"videos": videos, "task_id": "", "raw": created}
        raise HTTPException(status_code=502, detail=f"豆包账号池接口服务未返回 requestId：{created}")

    deadline = time.monotonic() + min(float(VIDEO_POLL_TIMEOUT), 1800.0)
    last = created
    recovery_requested = False
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=10.0, read=30.0, write=20.0, pool=10.0), follow_redirects=True, trust_env=False) as client:
        while time.monotonic() < deadline:
            await asyncio.sleep(3)
            response = await client.get(f"{base_url}/api/requests/{urllib.parse.quote(request_id, safe='')}", headers=doubao_pool_headers(provider))
            if response.status_code == 401:
                raise HTTPException(status_code=401, detail="豆包账号池接口服务 API Key 不正确。")
            if response.status_code >= 400:
                raise HTTPException(status_code=response.status_code, detail=f"查询豆包视频任务失败：{response.text[:800]}")
            current = response.json()
            last = current
            status = str((current or {}).get("status") or "").strip().lower()
            if status in {"success", "succeeded", "completed", "complete", "done"}:
                urls = doubao_pool_result_values(current)
                if not urls:
                    clean_url = str((current or {}).get("cleanVideoUrl") or (current or {}).get("clean_video_url") or "").strip()
                    raw_url = str((current or {}).get("rawVideoUrl") or (current or {}).get("raw_video_url") or (current or {}).get("doubaoThreadUrl") or "").strip()
                    raise HTTPException(status_code=502, detail=f"豆包账号池标记任务成功，但没有返回可播放视频。cleanVideoUrl={clean_url or '空'}；rawVideoUrl={raw_url or '空'}")
                local_urls = [await normalize_doubao_pool_video_output(url) for url in urls]
                videos = [u for u in local_urls if u]
                if not videos:
                    raise HTTPException(status_code=502, detail=f"豆包账号池返回了视频链接，但画布下载保存失败：{urls[0]}")
                return {"videos": videos, "task_id": request_id, "raw": current}
            if status in {"failed", "error", "stopped", "cancelled", "canceled"}:
                failure_message = str((current or {}).get("message") or "").strip()
                recoverable_markers = (
                    "未提取到豆包分享链接",
                    "未拿到豆包分享链接",
                    "复制链接失败",
                ...
                if not recovery_requested and any(marker in failure_message for marker in recoverable_markers):
                    recovery_response = await client.post(
                        f"{base_url}/api/requests/{urllib.parse.quote(request_id, safe='')}/retry-result",
                        headers=doubao_pool_headers(provider),
                    )
                    if recovery_response.status_code < 400:
                        recovery_requested = True
                        try:
                            last = recovery_response.json()
                        except Exception:
                            last = {"status": "recovering", "requestId": request_id}
                        continue
                    raise HTTPException(
                        status_code=502,
                        detail=f"豆包视频任务失败且结果恢复未启动：{failure_message or current}；恢复接口：{recovery_response.text[:500]}",
                    )
                raise HTTPException(status_code=502, detail=f"豆包视频任务失败：{failure_message or current}")
    raise HTTPException(status_code=504, detail=f"豆包视频任务超时：{request_id}；最后状态：{last}")
...
async def test_provider_connection(payload: TestConnectionPayload):
    ...
    if protocol == "doubao-pool":
        base_url = (payload.base_url or "http://127.0.0.1:17888").strip().rstrip("/")
        try:
            ...
            return {
                ...
                "status": 0,
                "protocol": "doubao-pool",
                "message": f"无法连接豆包账号池接口服务：{base_url}。请先打开“豆包账号池接口服务.app”，并确认本地 API 已开启。",
                "raw": {"error": str(exc)[:300], "base_url": base_url},
            }
        return {
            "ok": bool(raw.get("ok")),
            "status": 200,
            "protocol": "doubao-pool",
            "message": "豆包账号池接口服务已连接。",
            "model_count": 2,
            "image_models": [],
            "chat_models": [],
            "video_models": ["seedance_2_0_mini", "seedance_2_0_fast"],
            "all": ["seedance_2_0_mini", "seedance_2_0_fast"],
            "raw": raw,
        }
    ...
    if protocol == "doubao-pool":
        ...
        try:
            ...
            return {
                ...
                "protocol": "doubao-pool",
                "status_code": 0,
                "message": f"无法连接豆包账号池接口服务：{base_url}",
                "raw": {"error": str(exc)[:300], "base_url": base_url},
            }
        return {
            "ok": bool(raw.get("ok")),
...
async def build_online_image_result(payload: OnlineImageRequest):
    ...
    payload_width, payload_height = parse_size_pair(payload_size)
    if (not payload_width or not payload_height) and payload.width > 0 and payload.height > 0:
        payload_size = f"{payload.width}x{payload.height}"
    request_size = snap_size_to_multiple(payload_size, 16)
    ...
    async def generate_one():
        ...
        for item in image_items:
            ...
            if local_url:
                local_url = postprocess_output_url_to_requested_size(local_url, request_size, provider["id"])
            if local_url:
...
async def canvas_video(payload: CanvasVideoRequest):
    ...
    if is_doubao_pool_provider(provider):
        return await generate_doubao_pool_video(payload, provider)
    if is_runninghub_provider(provider):
        try:
...
async def optimize_prompt(payload: OptimizePromptRequest):
    """使用 LLM 优化图像生成提示词。画框「AI 优化」按钮调用此端点。"""
    system_prompt = (payload.system_prompt or "").strip() or OPTIMIZE_PROMPT_SYSTEM
    provider = (payload.provider or "").strip()
    model = (payload.model or "").strip()
    if not provider:
        raise HTTPException(status_code=400, detail="未指定接口平台（provider）")
    # 处理 CLI 类型 provider
    _provider = get_api_provider(provider)
    if is_codex_provider(_provider):
        chat_payload = CanvasLLMRequest(
            message=payload.prompt,
            system_prompt=system_prompt,
            model=model or (_provider.get("chat_models") or CODEX_DEFAULT_CHAT_MODELS)[0],
            messages=[],
            provider=provider,
        )
        text, _ = await codex_chat_text(chat_payload, [])
        return {"optimized_prompt": (text or "").strip()}
    if is_gemini_cli_provider(_provider):
        chat_payload = CanvasLLMRequest(
            message=payload.prompt,
            system_prompt=system_prompt,
            model=model or (_provider.get("chat_models") or GEMINI_CLI_DEFAULT_CHAT_MODELS)[0],
            messages=[],
            provider=provider,
        )
        text, _ = await gemini_cli_chat_text(chat_payload, [])
        return {"optimized_prompt": (text or "").strip()}
    # 通用 OpenAI 兼容接口
    chat_base, chat_hdrs, resolved_model = resolve_chat_provider(provider, model, "")
    _llm_provider = get_api_provider(provider) if provider not in ("modelscope",) else {}
    _is_apimart = is_apimart_provider(_llm_provider)
    upstream_messages = [
    ...
    try:
        ...
        body = exc.response.text or ""
        friendly = friendly_chat_error_detail(body, resolved_model, _llm_provider)
        raise HTTPException(status_code=exc.response.status_code, detail=friendly or f"上游接口错误：{body}") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"请求上游接口失败：{exc}") from exc
    except HTTPException:
    ...
    try:
        text = text_from_chat_response(raw).strip() if isinstance(raw, dict) else ""
        if not text:
            raise HTTPException(status_code=502, detail="AI 未返回优化结果")
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"解析回复内容失败：{exc}") from exc
