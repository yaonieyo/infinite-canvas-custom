...
APIMART_DEFAULT_VIDEO_MODELS = [
    # APIMart 官方视频目录的可提交 model ID（展示名与 API ID 不完全相同）。
...
def windows_system_proxy_url():
    ...
    try:
        ...
        if ";" in server:
            ...
            server = parts.get("https") or parts.get("http") or next((v for v in parts.values() if v), "")
        if not server:
            return ""
...
def gpt_image_2_skill_size_arg(size="", model="", prompt="", provider="openai"):
    ...
    if str(provider or "").strip().lower() == "codex":
        ...
        if width and height:
            return normalize_gpt_image_2_size(f"{width}x{height}") or ("2K" if max(width, height) < 3000 else "4K")
...
async def generate_codex_provider_image_via_gpt_image_2_skill(prompt, size, model, ref_paths=None):
    ...
    for attempt_index, (attempt_provider_args, attempt_provider) in enumerate(attempts):
        ...
        try:
            print(f"[gpt-image-2-skill] command={command_prefix} provider={attempt_provider} mode={mode} has_out={'--out' in args} ref_count={len(ref_paths)} out={out_path}")
            proc = await asyncio.create_subprocess_exec(
                *args,
                cwd=BASE_DIR,
                env=codex_subprocess_env(),
...
def output_file_from_url(url):
    ...
    elif clean.startswith("/assets/input/"):
        root = OUTPUT_INPUT_DIR
        rel = clean[len("/assets/input/"):]
