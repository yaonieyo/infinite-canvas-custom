...
let promptMentionUndoArmed = false;
let activeMentionInputEl = null;
let panState = null;
let didPan = false;
...
let canvasAssetMentionItems = [];
let canvasAssetMentionLoadedAt = 0;
...
const SIZE_MAP = {
    ...
    poster45: {'1k':'1024x1280','2k':'1632x2048','4k':'3072x3840'},
    landscape54: {'1k':'1280x1024','2k':'2048x1632','4k':'3840x3072'},
...
function refreshIcons(root=null){
    ...
    const toPascal = name => String(name || '')
        .replace(/^([A-Z])|[\s-_]+(\w)/g, (_, first, letter) => letter ? letter.toUpperCase() : String(first || '').toLowerCase())
    ...
    root.querySelectorAll?.('i[data-lucide]').forEach(el => {
        ...
        const classes = ['lucide', `lucide-${name}`, attrs.class || ''].filter(Boolean).join(' ');
        attrs.class = classes;
        attrs['data-lucide'] = name;
        if(!Object.keys(attrs).some(key => key.startsWith('aria-') || key === 'role' || key === 'title')) attrs['aria-hidden'] = 'true';
        el.replaceWith(window.lucide.createElement(icon, attrs));
    });
}
...
function isPosterFrameNode(node){
    return Boolean(node && node.type === 'smart-frame');
}
function isPosterFrameBatchNode(node){
    return Boolean(node && node.type === 'smart-frame-batch');
}
function isSmartRunnableNode(node){
    return Boolean(isSmartImageNode(node) || isSmartGroupNode(node) || node?.type === 'image-prompt-card' || (node?.type === 'storyboard-card' && node?.cardKind !== 'storyboard'));
}
...
function promptImageRefsFromParts(parts=collectPromptParts()){
    const seen = new Set();
    return (parts || [])
        .filter(part => part?.type === 'image' && part.assetId && part.url)
        .map(part => ({
...
function syncPromptImageBindingsForNode(node){
    if(!node || !promptInput) return [];
    const refs = promptImageRefsFromParts();
    if(refs.length) node.promptImageRefs = refs;
...
function clearPromptInput(options={}){
    ...
    if(activeComposerSubject){
        ...
        delete activeComposerSubject.promptImageRefs;
    }
}
...
function syncSelectionUi(){
    ...
    smartNodeElementsByIds(touchedIds).forEach(el => {
        ...
        el.querySelectorAll('.thumb-item,.image-wrap,.storyboard-output-media').forEach(item => {
...
function smartGroupCompactMembers(node){
    return smartGroupMembers(node).filter(member => member?.type === 'smart-prompt' || member?.type === 'smart-loop' || member?.type === 'smart-frame' || member?.type === 'smart-frame-batch');
}
function isSmartGroupCompactMember(node){
    return Boolean(node && (node.type === 'smart-prompt' || node.type === 'smart-loop' || node.type === 'smart-frame' || node.type === 'smart-frame-batch') && smartGroupContainingNode(node.id));
}
...
function promptTextItemsForNode(node, ctx=smartLoopContext){
    ...
    if(node.type === 'image-prompt-card'){
        const text = String(node.promptText || node.runPrompt || node.promptDraftText || '').trim();
        return text ? [text] : [];
    }
    if(node.type === 'storyboard-card' && node.cardKind !== 'storyboard'){
        const shot = node.shot || {};
        const text = [shot.breakdown?.story, shot.breakdown?.purpose, shot.visual?.subject, shot.visual?.action, shot.visual?.scene, shot.visual?.focus].filter(Boolean).join('\n');
        return text ? [text] : [];
    }
...
function imageLayout(images, scale=1, node=null){
    ...
    if(['script-storyboard','storyboard-group','storyboard-card','image-prompt-card','asset-hub','shot-asset-collector'].includes(node?.type)){
        const isShotStoryCard = node?.type === 'storyboard-card' && node?.cardKind === 'storyboard';
        const isAssetHub = node?.type === 'asset-hub';
        const isShotAssetCollector = node?.type === 'shot-asset-collector';
        return {
            cols:1,
            rows:1,
            width:Math.round(Number(node.w) || (isShotAssetCollector ? 560 : isAssetHub ? 420 : isShotStoryCard ? 560 : node.type === 'storyboard-group' ? 230 : node.type === 'storyboard-card' ? 310 : node.type === 'image-prompt-card' ? 360 : 430)),
            height:Math.round(Number(node.h) || (isShotAssetCollector ? 650 : isAssetHub ? 520 : isShotStoryCard ? 600 : node.type === 'storyboard-group' ? 132 : node.type === 'storyboard-card' ? 270 : node.type === 'image-prompt-card' ? 430 : 610)),
            thumb:96,
            single:true
        };
    }
    if(node?.type === 'smart-prompt') return {cols:1, rows:1, ...promptNodeLayoutSize(node), thumb:96, single:true};
    if(node?.type === 'smart-frame') return {cols:1, rows:1, width:Math.round(Number(node.w) || 360), height:Math.round(Number(node.h) || 440), thumb:96, single:true};
    if(node?.type === 'smart-frame-batch') return {cols:1, rows:1, width:Math.round(Number(node.w) || 420), height:Math.round(Number(node.h) || 560), thumb:96, single:true};
    if(node?.type === 'smart-loop'){
        const explicitW = Number(node.w);
        const explicitH = Number(node.h);
        if(isSmartGroupCompactMember(node) && Number.isFinite(explicitW) && explicitW > 24 && Number.isFinite(explicitH) && explicitH > 24){
    ...
    if(node?.type === 'smart-image' && node.storyboardOutputKind){
        const isVideo = node.storyboardOutputKind === 'video' || node.runSettings?.apiKind === 'video';
        return {cols:1, rows:1, width:Math.round(Number(node.w) || 560), height:Math.round(Number(node.h) || (isVideo ? 620 : 650)), thumb:96, single:true};
    }
    const count = (images || []).length;
...
function posterFrameChatProviderId(providerId=''){
    const providers = chatApiProviders();
    const usable = providers.filter(p => p.id !== 'modelscope' || configHasModelscopeKey);
    if(usable.some(p => p.id === providerId)) return providerId;
    if(usable.some(p => p.id === 'codex')) return 'codex';
    if(providers.some(p => p.id === 'codex')) return 'codex';
    return usable[0]?.id || providers[0]?.id || 'codex';
}
...
function renderVideoDurationControl(){
    const locked = hasFixedVideoDuration(settings.videoProvider);
    const v = normalizeSmartVideoDuration(settings.videoDuration, settings.videoProvider);
    if(locked) settings.videoDuration = v;
    const quick = [3, 4, 5, 6, 8, 10, 12, 15];
    const options = locked ? [5, 10] : quick;
    return `<div class="smart-control duration-control" title="${escapeHtml(tr('smart.videoDurationTip'))}">
        ...
        <div class="smart-popover compact-popover">
            ...
            <div class="duration-grid">
                ${options.map(n => `<button type="button" class="duration-option ${n === v ? 'active' : ''}" data-smart-param="videoDuration" data-smart-value="${n}">${n}s</button>`).join('')}
            </div>
            <label class="duration-custom" ${locked ? 'hidden' : ''}>
...
function sourceRatioImageForNode(node){
    let images = (node?.images || []).filter(img => img?.url && !isAudioMediaItem(img));
    if(!images.length && (node?.type === 'image-prompt-card' || node?.type === 'storyboard-card')){
        images = defaultReferenceImagesFor(node).filter(img => img?.url && !isAudioMediaItem(img));
    }
...
function setDynamicSetting(key, value){
    ...
    if(key === 'videoProvider' || key === 'videoDuration') settings.videoDuration = normalizeSmartVideoDuration(settings.videoDuration, settings.videoProvider);
...
async function loadConfig(){
    try {
        ...
        configHasModelscopeKey = cfg.has_ms_key !== false;
...
function createPosterFrameNode(x, y, options={}){
    if(!options.skipUndo) pushUndo();
    const prompt = String(options.prompt || '').trim();
    const title = String(options.title || '').trim() || '产品海报';
    const runSettings = posterFrameNormalizedRunSettings(options.runSettings || settings);
    const node = {
        id:uid('frame'),
        type:'smart-frame',
        x,
        y,
        w:Number(options.w) || 380,
        h:Number(options.h) || 520,
        title,
        frameTitle:title,
        prompt,
        sizeHint:String(options.sizeHint || options.ratio || '').trim() || '海报',
        images:[],
        runSettings:settingsForStorage(runSettings),
        created_at:Date.now()
    };
    nodes.push(node);
    if(options.select !== false) selectedId = node.id;
...
function createPosterFrameBatchNode(x, y, options={}){
    if(!options.skipUndo) pushUndo();
    const provider = posterFrameChatProviderId(options.splitProvider || '');
    const node = {
        id:uid('framebatch'),
        type:'smart-frame-batch',
        x,
        y,
        w:Number(options.w) || 420,
        h:Number(options.h) || 560,
        title:String(options.title || '').trim() || '批量画框',
        batchText:String(options.batchText || ''),
        splitProvider:provider,
        splitModel:resolveChatModel(options.splitModel || '', provider),
        splitOutput:'',
        splitStatus:'',
        createdFrameIds:[],
        created_at:Date.now()
...
function transplantSmartMediaElements(oldNodeEl, newNodeEl){
    const oldItems = [...(oldNodeEl?.querySelectorAll?.('.thumb-item,.image-wrap,.storyboard-output-media') || [])];
    const newItems = [...(newNodeEl?.querySelectorAll?.('.thumb-item,.image-wrap,.storyboard-output-media') || [])];
...
function promptNodeBodyHtml(node){
    ...
    const readonly = node.llmEnabled ? 'false' : 'true';
    ...
    const promptTextHtml = promptHtmlWithMentionTokens(node.text || '', node.promptImageRefs || []) || escapeHtml(node.text || '');
    return `<div class="prompt-node-card">
        <div class="prompt-node-text prompt-node-control" contenteditable="${readonly}" data-prompt-node-id="${escapeHtml(node.id || '')}" data-placeholder="${escapeHtml(tr('smart.promptPlaceholderNode'))}">${promptTextHtml}</div>
...
function posterFrameNormalizedRunSettings(source=settings){
    const providers = imageProviders();
    const providerId = providers.some(p => p.id === source?.provider_id) ? source.provider_id : (providers[0]?.id || source?.provider_id || '');
    const models = providerImageModels(providerId);
    const model = models.includes(source?.model) ? source.model : (models[0] || source?.model || '');
    const ratio = SIZE_MAP[source?.ratio] ? source.ratio : 'portrait43';
    const allowAuto = isGptImageAutoSizeModel(model);
    const resolution = source?.resolution && (allowAuto || source.resolution !== 'auto') ? source.resolution : (allowAuto ? defaultSmartApiResolution(model) : '1k');
    return {
...
function posterFrameImageProviderOptions(selectedId=''){
    const providers = imageProviders();
    const selected = providers.some(p => p.id === selectedId) ? selectedId : (providers[0]?.id || selectedId || '');
...
function posterFrameImageModelOptions(providerId='', selectedModel=''){
    const models = providerImageModels(providerId);
    const selected = models.includes(selectedModel) ? selectedModel : (models[0] || selectedModel || '');
...
function posterFrameRatioOptions(selectedRatio=''){
    const ratios = [
        ['square','1:1'],
        ['poster45','4:5'],
        ['portrait43','3:4'],
    ...
    const selected = SIZE_MAP[selectedRatio] ? selectedRatio : 'portrait43';
    return ratios.map(([value, label]) => `<option value="${escapeAttr(value)}" ${value === selected ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('');
}
function posterFrameResolutionOptions(selectedResolution='', selectedModel=''){
    const allowAuto = isGptImageAutoSizeModel(selectedModel);
    const options = allowAuto ? ['auto','1k','2k','4k'] : ['1k','2k','4k'];
    const selected = options.includes(selectedResolution) ? selectedResolution : (allowAuto ? defaultSmartApiResolution(selectedModel) : '1k');
    return options.map(value => `<option value="${escapeAttr(value)}" ${value === selected ? 'selected' : ''}>${escapeHtml(value === 'auto' ? '自动' : value.toUpperCase())}</option>`).join('');
}
function posterFrameRunSizeLabel(runSettings){
    const ratioMap = {
        square:'1:1',
        poster45:'4:5',
        portrait43:'3:4',
    ...
    const ratio = ratioMap[runSettings?.ratio || ''] || '1:1';
    const resolution = String(runSettings?.resolution || '').toUpperCase() || '1K';
...
function posterFrameRatioFromText(text){
    ...
    if(!value) return '';
    if(/9\s*[:：]\s*16|竖版|手机海报|小红书|story/.test(value)) return 'story';
    if(/4\s*[:：]\s*5|四比五/.test(value)) return 'poster45';
    if(/3\s*[:：]\s*4/.test(value)) return 'portrait43';
    if(/2\s*[:：]\s*3/.test(value)) return 'portrait';
    if(/16\s*[:：]\s*9|横版|宽屏|banner/.test(value)) return 'wide';
    if(/5\s*[:：]\s*4/.test(value)) return 'landscape54';
    if(/4\s*[:：]\s*3/.test(value)) return 'landscape43';
    if(/3\s*[:：]\s*2/.test(value)) return 'landscape';
    if(/21\s*[:：]\s*9/.test(value)) return 'ultrawide';
    if(/9\s*[:：]\s*21/.test(value)) return 'ultratall';
    if(/1\s*[:：]\s*1|方图|方形|square/.test(value)) return 'square';
    return '';
}
function posterFrameImageUrlsFromText(text){
    const value = String(text || '');
    const matches = value.match(/(?:https?:\/\/|\/(?:assets|output|static)\/)[^\s"'<>，。；、)）\]]+/ig) || [];
    return [...new Set(matches.map(url => url.trim()).filter(url => looksLikeImageMediaUrl(url)))];
}
function posterFrameReferenceImages(node){
    if(!node) return [];
    const textRefs = posterFrameImageUrlsFromText([node.prompt, node.batchText].filter(Boolean).join('\n')).map((url, index) => ({
        url,
        name:smartImageNameFromUrl(url) || `链接图${index + 1}`,
        kind:'image',
        manualAdded:true
    }));
    return uniqueReferenceImages([
        ...inputImagesFor(node).filter(img => img?.url),
        ...manualReferenceImagesFor(node),
        ...textRefs
    ]);
}
function posterFrameReferenceThumbsHtml(node){
    const refs = posterFrameReferenceImages(node);
    if(!refs.length) return '';
    return smartNodeInputThumbsHtml(refs, {labelPrefix:'参考'});
}
function posterFrameAiSplitPromptPreview(){
    return [
        '内置 AI 拆分规则：',
        '1. 先判断用户到底写了多少张完整海报，不按段落/字段硬拆。',
        '2. 每张提示词卡必须是一张完整产品海报 prompt，不能把视觉主题、背景、产品、文案拆成多张。',
        '3. 只返回 JSON 数组：[{"title":"产品海报 1","prompt":"完整生图提示词","size":"建议画幅"}]。'
    ].join('\n');
}
function posterFrameBatchBodyHtml(node){
    const running = Boolean(node.running);
    const provider = posterFrameChatProviderId(node.splitProvider || '');
    const model = resolveChatModel(node.splitModel || '', provider);
    const liveCount = nodes.filter(item => item?.type === 'smart-frame' && (item.sourceBatchNodeId === node.id || (Array.isArray(node.createdFrameIds) && node.createdFrameIds.includes(item.id)))).length;
    const outputText = String(node.batchText || '').trim()
        ? (node.splitOutput || node.splitStatus || posterFrameAiSplitPromptPreview())
        : posterFrameAiSplitPromptPreview();
    return `<div class="poster-frame-batch-card">
        <div class="poster-frame-batch-top">
            <input class="poster-frame-batch-title" type="text" value="${escapeAttr(node.title || '批量画框')}" placeholder="节点标题">
            <span class="poster-frame-batch-count">${escapeHtml(liveCount ? `${liveCount} 张` : '未拆分')}</span>
        </div>
        <textarea class="poster-frame-batch-input" placeholder="粘贴你的多张产品海报提示词；写多少张，就拆成多少张提示词卡。">${escapeHtml(node.batchText || '')}</textarea>
        <div class="poster-frame-batch-controls">
            <label><span>接口</span><select class="poster-frame-batch-provider">${chatProviderOptions(provider)}</select></label>
            <label><span>模型</span><select class="poster-frame-batch-model">${chatModelOptions(model, provider)}</select></label>
        </div>
        <div class="poster-frame-batch-output">${escapeHtml(outputText)}</div>
        <div class="poster-frame-batch-actions">
            <button class="poster-frame-batch-split primary" type="button" data-poster-batch-split="ai" ${running ? 'disabled' : ''}><i data-lucide="${running ? 'loader-2' : 'sparkles'}"></i><span>${running ? '拆分中' : '生成提示词卡'}</span></button>
        </div>
    </div>`;
}
function posterFrameBodyHtml(node){
    const runSettings = posterFrameNormalizedRunSettings(node.runSettings || settings);
    node.runSettings = settingsForStorage(runSettings);
    const refThumbs = posterFrameReferenceThumbsHtml(node);
    ...
    const promptHtml = promptHtmlWithMentionTokens(node.prompt || '', node.promptImageRefs || []) || escapeHtml(node.prompt || '');
    return `<div class="poster-frame-card">
        <div class="poster-frame-top">
            <input class="poster-frame-title" type="text" value="${escapeAttr(node.frameTitle || node.title || '产品海报')}" placeholder="画框标题">
            <span class="poster-frame-size">${escapeHtml(node.sizeHint || '海报')}</span>
        </div>
        <div class="poster-frame-prompt" contenteditable="true" data-poster-frame-id="${escapeHtml(node.id || '')}" data-placeholder="这张海报的生图提示词">${promptHtml}</div>
        <div class="poster-frame-prompt-actions">
            <button class="poster-frame-optimize" type="button" data-poster-frame-optimize="${escapeAttr(node.id)}" ${running ? 'disabled' : ''}><i data-lucide="wand-2"></i><span>AI 优化</span></button>
        </div>
        <div class="poster-frame-controls">
            <label><span>接口</span><select class="poster-frame-provider">${posterFrameImageProviderOptions(runSettings.provider_id || '')}</select></label>
            <label><span>模型</span><select class="poster-frame-model">${posterFrameImageModelOptions(runSettings.provider_id || '', runSettings.model || '')}</select></label>
            <label><span>画幅</span><select class="poster-frame-ratio">${posterFrameRatioOptions(runSettings.ratio || '')}</select></label>
            <label><span>清晰度</span><select class="poster-frame-resolution">${posterFrameResolutionOptions(runSettings.resolution || '', runSettings.model || '')}</select></label>
        </div>
        ${refThumbs ? `<div class="poster-frame-refs">${refThumbs}</div>` : ''}
        ${output}
        <div class="poster-frame-actions-row">
            ${meta}
            <button class="poster-frame-run" type="button" data-poster-frame-run="${escapeAttr(node.id)}" ${running ? 'disabled' : ''}><i data-lucide="${running ? 'loader-2' : 'sparkles'}"></i><span>${running ? '生成中' : '立即生图'}</span></button>
        </div>
    </div>`;
}
function nodeBodyHtml(node, layout){
    if(node.type === 'smart-group') return smartGroupBodyHtml(node);
    if(node.type === 'script-storyboard') return window.smartStoryboardInputBodyHtml ? window.smartStoryboardInputBodyHtml(node) : '';
    if(node.type === 'storyboard-group') return window.smartStoryboardGroupBodyHtml ? window.smartStoryboardGroupBodyHtml(node) : '';
    if(node.type === 'storyboard-card' && node.cardKind === 'storyboard') return window.smartStoryboardCardBodyHtml ? window.smartStoryboardCardBodyHtml(node) : '';
    if(node.type === 'storyboard-card') return window.smartStoryboardShotCardBodyHtml ? window.smartStoryboardShotCardBodyHtml(node) : '';
    if(node.type === 'image-prompt-card') return window.smartImagePromptCardBodyHtml ? window.smartImagePromptCardBodyHtml(node) : '';
    if(node.type === 'asset-hub') return window.smartAssetHubBodyHtml ? window.smartAssetHubBodyHtml(node) : '';
    if(node.type === 'shot-asset-collector') return window.shotAssetCollectorBodyHtml ? window.shotAssetCollectorBodyHtml(node) : '';
    if(node.type === 'smart-prompt') return promptNodeBodyHtml(node);
    if(node.type === 'smart-frame-batch') return posterFrameBatchBodyHtml(node);
    if(node.type === 'smart-frame') return posterFrameBodyHtml(node);
    if(node.type === 'smart-loop') return smartLoopBodyHtml(node);
    if(node.type === 'smart-image' && node.storyboardOutputKind && window.smartStoryboardOutputNodeBodyHtml) return window.smartStoryboardOutputNodeBodyHtml(node, layout);
...
function render(){
    ...
    const nodeHtmlEntries = nodes
        ...
        const title = node.type === 'script-storyboard' ? (node.title || '剧转图') : node.type === 'storyboard-group' ? (node.shotNumber || '故事板') : node.type === 'storyboard-card' ? (node.cardKind === 'storyboard' ? (node.shot?.shotNumber || '故事板') : (node.shotNumber || '镜头卡')) : node.type === 'image-prompt-card' ? (node.title || '生图提示词') : node.type === 'asset-hub' ? (node.title || '资产中枢') : node.type === 'shot-asset-collector' ? (node.title || '人物收集器') : node.type === 'smart-group' ? (node.title === '万能分组' ? '智能分组' : (node.title || '智能分组')) : node.type === 'smart-prompt' ? 'Prompt' : node.type === 'smart-loop' ? 'Loop' : node.type === 'smart-frame-batch' ? (node.title || '批量画框') : node.type === 'smart-frame' ? (node.frameTitle || node.title || '画框') : (imgs.length > 1 ? 'Group' : imgs.length ? 'Image' : escapeHtml(tr('smart.createImportNode')));
        const scale = nodeScale(node);
        const layout = imageLayout(imgs, scale, node);
        const isPrompt = node.type === 'smart-prompt';
        const isLoop = node.type === 'smart-loop';
        const isPosterFrame = node.type === 'smart-frame';
        const isPosterFrameBatch = node.type === 'smart-frame-batch';
        const isSmartGroup = node.type === 'smart-group';
        const isScriptStoryboard = node.type === 'script-storyboard';
        const isStoryboardGroup = node.type === 'storyboard-group';
        const isStoryboardCard = node.type === 'storyboard-card';
        const isImagePromptCard = node.type === 'image-prompt-card';
        const isAssetHub = node.type === 'asset-hub';
        const isShotAssetCollector = node.type === 'shot-asset-collector';
        const isStoryNode = isScriptStoryboard || isStoryboardGroup || isStoryboardCard || isImagePromptCard || isAssetHub || isShotAssetCollector;
        const isCompactMember = isSmartGroupCompactMember(node);
        const isImageNode = node.type === 'smart-image' || !node.type;
        const isStoryboardOutput = isImageNode && Boolean(node.storyboardOutputKind);
        ...
        const isEmpty = isImageNode && !isStoryboardOutput && imgs.length === 0 && !node.pending && !isQueued && !isJimengPending;
        ...
        const hint = isScriptStoryboard ? '粘贴长剧本 · 生成故事板' : isShotAssetCollector ? '绑定人物/场景资产 · 生成前检查' : isStoryboardCard ? '故事板卡 · 可生成图或视频提示词' : isAssetHub ? '汇总全剧参考资产' : isImagePromptCard ? '故事板生图提示词' : isPosterFrameBatch ? '粘贴多张提示词 · 生成提示词卡' : isPosterFrame ? '配置接口模型画幅 · 点击立即生图' : isSmartGroup ? '双击添加 · 拖入归组 · 选中后生成' : isPending ? escapeHtml(tr('smart.hintPending')) : (imgs.length > 1 ? escapeHtml(tr('smart.hintMulti')) : imgs.length ? escapeHtml(tr('smart.hintSingle')) : escapeHtml(tr('smart.hintEmpty')));
        const html = `<div class="image-node ${isEmpty ? 'empty-node' : ''} ${isGroup ? 'group-node' : ''} ${isHistory ? 'history-group-node' : ''} ${isPrompt ? 'prompt-smart-node' : ''} ${isLoop ? 'loop-smart-node' : ''} ${isPosterFrame ? 'poster-frame-node' : ''} ${isPosterFrameBatch ? 'poster-frame-batch-node' : ''} ${isSmartGroup ? 'smart-group-node' : ''} ${isScriptStoryboard ? 'script-storyboard-node' : ''} ${isStoryboardGroup ? 'storyboard-group-node' : ''} ${isStoryboardCard ? 'storyboard-card-node' : ''} ${isImagePromptCard ? 'image-prompt-card-node' : ''} ${isAssetHub ? 'asset-hub-node' : ''} ${isShotAssetCollector ? 'shot-asset-collector-node' : ''} ${isStoryboardOutput ? 'storyboard-output-node' : ''} ${isCompactMember ? 'smart-group-member-node' : ''} ${isNodeSelected(node.id) ? 'selected' : ''} ${(dragState?.groupIds?.includes(node.id) || dragState?.id === node.id) ? 'dragging' : ''} ${node.running ? 'node-running' : ''} ${isPending ? 'node-pending' : ''}" data-id="${escapeHtml(node.id)}" style="left:${node.x || 0}px;top:${node.y || 0}px;width:${layout.width}px;height:${layout.height}px">
            ...
            ${isCompactMember && (isPrompt || isLoop || isPosterFrame || isPosterFrameBatch || isStoryNode) ? '<div class="smart-group-member-grab" title="拖动移出分组"></div>' : ''}
            <div class="node-hint">${hint}</div>
            ${imgs.length || node.pending || isQueued || isJimengPending || isPrompt || isLoop || isPosterFrame || isPosterFrameBatch || isSmartGroup || isStoryNode ? '<div class="node-resize-handle" data-resize="1"></div>' : ''}
...
function syncPromptNodeTextFromElement(node, textEl){
    if(!node || !textEl) return;
    node.text = originalPromptTextFromParts(collectPromptParts(textEl));
    const refs = promptImageRefsFromParts(collectPromptParts(textEl));
    if(refs.length) node.promptImageRefs = refs;
    else delete node.promptImageRefs;
}
function syncPosterFramePromptFromElement(node, promptEl){
    if(!node || !promptEl) return;
    node.prompt = originalPromptTextFromParts(collectPromptParts(promptEl));
    const refs = promptImageRefsFromParts(collectPromptParts(promptEl));
    if(refs.length) node.promptImageRefs = refs;
    else delete node.promptImageRefs;
...
function bindPromptNodeMentionInput(textEl, node){
    ...
    textEl.addEventListener('mouseup', () => { activate(); saveMentionRange(); });
    textEl.addEventListener('keyup', () => { activate(); maybeOpenMentionPicker(); });
    textEl.addEventListener('input', () => {
        activate();
        promptMentionUndoArmed = false;
        maybeOpenMentionPicker();
    });
    textEl.addEventListener('keydown', event => {
        activate();
        const key = String(event.key || '').toLowerCase();
        if((event.ctrlKey || event.metaKey) && key === 'z' && promptMentionUndoArmed && undoStack.length){
            event.preventDefault();
            promptMentionUndoArmed = false;
            performUndo();
            setTimeout(() => {
...
function bindPosterFrameMentionInput(promptEl, node){
    ...
    promptEl.addEventListener('mouseup', () => { activate(); saveMentionRange(); });
    promptEl.addEventListener('keyup', () => { activate(); maybeOpenMentionPicker(); });
...
function bindPromptNodeControls(el, node){
    ...
    if(textEl) {
        ...
        bindPromptNodeMentionInput(textEl, node);
        textEl.oninput = e => {
            const prevExtra = promptNodeSplitExtraHeight(node);
            syncPromptNodeTextFromElement(node, textEl);
...
function bindPosterFrameControls(el, node){
    el.querySelectorAll('.poster-frame-title,.poster-frame-prompt,.poster-frame-run,.poster-frame-output,.poster-frame-provider,.poster-frame-model,.poster-frame-ratio,.poster-frame-resolution').forEach(control => {
        control.addEventListener('mousedown', e => e.stopPropagation());
        control.addEventListener('click', e => e.stopPropagation());
        control.addEventListener('dblclick', e => e.stopPropagation());
    });
    node.runSettings = settingsForStorage(posterFrameNormalizedRunSettings(node.runSettings || settings));
    const titleEl = el.querySelector('.poster-frame-title');
    if(titleEl) titleEl.oninput = e => {
        node.frameTitle = e.target.value;
        node.title = e.target.value || '画框';
        scheduleSave();
    };
    const promptEl = el.querySelector('.poster-frame-prompt');
    if(promptEl){
        bindScrollableText(promptEl);
        bindPosterFrameMentionInput(promptEl, node);
        promptEl.oninput = () => {
            syncPosterFramePromptFromElement(node, promptEl);
            scheduleSave();
        };
    }
    const providerEl = el.querySelector('.poster-frame-provider');
    const modelEl = el.querySelector('.poster-frame-model');
    const ratioEl = el.querySelector('.poster-frame-ratio');
    const resolutionEl = el.querySelector('.poster-frame-resolution');
    const updateSettings = (patch={}, rerender=false) => {
        const next = posterFrameNormalizedRunSettings({...node.runSettings, ...patch});
    ...
    if(providerEl) providerEl.onchange = e => {
        ...
        const models = providerImageModels(providerId);
        updateSettings({provider_id:providerId, model:models[0] || ''}, true);
    };
    if(modelEl) modelEl.onchange = e => updateSettings({model:e.target.value}, true);
    if(ratioEl) ratioEl.onchange = e => updateSettings({ratio:e.target.value}, true);
    if(resolutionEl) resolutionEl.onchange = e => updateSettings({resolution:e.target.value}, true);
    el.querySelectorAll('[data-poster-frame-run]').forEach(btn => {
        btn.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); }, true);
        btn.addEventListener('click', e => {
            e.preventDefault();
            e.stopPropagation();
            runPosterFrameNode(btn.dataset.posterFrameRun || node.id);
        });
    });
    el.querySelectorAll('[data-poster-frame-optimize]').forEach(btn => {
        btn.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); }, true);
        btn.addEventListener('click', e => {
            e.preventDefault();
            e.stopPropagation();
            optimizePosterFramePrompt(btn.dataset.posterFrameOptimize || node.id);
        });
    });
}
function bindPosterFrameBatchControls(el, node){
    el.querySelectorAll('.poster-frame-batch-title,.poster-frame-batch-input,.poster-frame-batch-provider,.poster-frame-batch-model,.poster-frame-batch-output,.poster-frame-batch-split').forEach(control => {
        control.addEventListener('mousedown', e => e.stopPropagation());
        control.addEventListener('click', e => e.stopPropagation());
        control.addEventListener('dblclick', e => e.stopPropagation());
    });
    const titleEl = el.querySelector('.poster-frame-batch-title');
    if(titleEl) titleEl.oninput = e => {
        node.title = e.target.value || '批量画框';
        scheduleSave();
    };
    const inputEl = el.querySelector('.poster-frame-batch-input');
    if(inputEl){
        bindScrollableText(inputEl);
        inputEl.oninput = e => {
            node.batchText = e.target.value;
            node.splitOutput = '';
            node.createdFrameIds = [];
            node.splitStatus = String(node.batchText || '').trim() ? '内容已更新，点击“生成提示词卡”调用 AI 拆分。' : '';
            scheduleSave();
            const outputEl = el.querySelector('.poster-frame-batch-output');
            if(outputEl) outputEl.textContent = node.splitStatus || posterFrameAiSplitPromptPreview();
            const countEl = el.querySelector('.poster-frame-batch-count');
            if(countEl) countEl.textContent = '未拆分';
        };
    }
    const providerEl = el.querySelector('.poster-frame-batch-provider');
    if(providerEl) providerEl.onchange = e => {
        node.splitProvider = posterFrameChatProviderId(e.target.value);
        node.splitModel = resolveChatModel('', node.splitProvider);
        scheduleSave();
        render();
    };
    const modelEl = el.querySelector('.poster-frame-batch-model');
    if(modelEl) modelEl.onchange = e => {
        node.splitModel = resolveChatModel(e.target.value, node.splitProvider || '');
        scheduleSave();
    };
    el.querySelectorAll('[data-poster-batch-split]').forEach(btn => {
        btn.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); }, true);
        btn.addEventListener('click', e => {
            e.preventDefault();
            e.stopPropagation();
            createPosterFrameCardsFromBatch(node.id, btn.dataset.posterBatchSplit !== 'raw');
        });
    });
}
...
function bindNodeEvents(){
    world.querySelectorAll('.image-node').forEach(el => {
        ...
        if(nodeForControls?.type === 'smart-frame-batch') bindPosterFrameBatchControls(el, nodeForControls);
        if(nodeForControls?.type === 'smart-frame') bindPosterFrameControls(el, nodeForControls);
        if(nodeForControls?.type === 'script-storyboard') window.bindScriptStoryboardControls?.(el, nodeForControls);
        if(nodeForControls?.type === 'storyboard-card' && nodeForControls.cardKind === 'storyboard') window.bindSmartStoryboardCardControls?.(el, nodeForControls);
        if(nodeForControls?.type === 'storyboard-card' && nodeForControls.cardKind !== 'storyboard') window.bindSmartStoryboardShotCardControls?.(el, nodeForControls);
        if(nodeForControls?.type === 'image-prompt-card') window.bindSmartImagePromptCardControls?.(el, nodeForControls);
        if(nodeForControls?.type === 'asset-hub') window.bindSmartAssetHubControls?.(el, nodeForControls);
        if(nodeForControls?.type === 'shot-asset-collector') window.bindShotAssetCollectorControls?.(el, nodeForControls);
        ...
        el.querySelectorAll('.thumb-item,.image-wrap,.storyboard-output-media').forEach(item => {
            ...
            item.addEventListener('click', e => {
                ...
                if(item.classList.contains('storyboard-output-media')){
                    clearImageClickTimer();
                    suppressImageClickUntil = Date.now() + 260;
                    selectedId = id;
                    selectedIds = [];
...
function savePromptDraftForCurrent(){
    ...
    if(subject.type === 'image-prompt-card') subject.promptText = subject.promptDraftText;
    syncPromptImageBindingsForNode(subject);
...
function loadPromptDraft(subject){
    ...
    } else if(subject?.type === 'image-prompt-card' && typeof subject.promptText === 'string'){
        const rebuilt = promptHtmlWithMentionTokens(subject.promptText, subject.promptImageRefs || subject.runPromptRefs || []);
        if(rebuilt) promptInput.innerHTML = rebuilt;
        else setPromptText(subject.promptText);
...
function updateComposer(){
    ...
    if(node?.type === 'smart-image' && node.storyboardOutputKind){
        ...
        window.renderComposerPromptTools?.(null);
        return;
    }
    ...
    setPromptInputLocked(subject.promptUserLocked === true);
    window.renderComposerPromptTools?.(subject);
...
function mentionTokenHtml(img){
    ...
    return `<span class="mention-image-token" contenteditable="false" data-url="${escapeHtml(img.url)}" data-kind="${escapeHtml(kind)}" data-name="${escapeHtml(name)}" data-asset-id="${escapeHtml(img.assetId || img.asset_id || img.id || '')}" data-node-id="${escapeHtml(img.nodeId || '')}" data-image-index="${escapeHtml(img.imageIndex ?? '')}">${media}<span>@${escapeHtml(name)}</span></span>`;
}
...
function textForNode(node, ctx=smartLoopContext){
    ...
    if(node.type === 'image-prompt-card') return String(node.promptText || node.runPrompt || node.promptDraftText || '').trim();
    if(node.type === 'storyboard-card' && node.cardKind !== 'storyboard') return promptTextItemsForNode(node, ctx).join('\n\n');
...
function promptInputNodesFor(node){
    return inputNodesFor(node).filter(input => input?.type === 'smart-prompt' || input?.type === 'smart-loop' || input?.type === 'smart-group' || input?.type === 'image-prompt-card' || input?.type === 'storyboard-card');
}
...
function defaultReferenceImagesFor(node, consume=false, ctx=smartLoopContext){
    ...
    if((node.type === 'image-prompt-card' || node.type === 'storyboard-card') && typeof window.smartStoryboardReferenceImagesFor === 'function'){
        const source = node.type === 'image-prompt-card' && typeof window.smartImagePromptSourceVisualCard === 'function'
            ? (window.smartImagePromptSourceVisualCard(node) || node)
            : node;
        storyboardRefs = window.smartStoryboardReferenceImagesFor(source, textForNode(node, ctx)).filter(img => img?.url);
    }
...
function inputMentionCandidateImages(node){
    const current = node?.type === 'smart-frame' ? posterFrameReferenceImages(node) : (node ? defaultReferenceImagesFor(node) : []);
    const canvasIdForMatch = canvas?.id || canvasId || '';
    const seen = new Set();
    return current.filter(img => {
        ...
        ...img,
        assetId:img.assetId || canvasAssetMentionItems.find(item => item.url === img.url && (!canvasIdForMatch || item.canvasId === canvasIdForMatch || item.canvas_id === canvasIdForMatch))?.assetId || canvasAssetMentionItems.find(item => item.url === img.url)?.assetId || '',
...
async function ensureCanvasAssetMentionItems(force=false){
    const now = Date.now();
    if(!force && canvasAssetMentionLoadedAt && now - canvasAssetMentionLoadedAt < 30000) return canvasAssetMentionItems;
    try {
        const data = await fetch('/api/canvas-assets').then(r => r.ok ? r.json() : null);
        const projectId = canvas?.project || sourceProjectId || 'default';
        canvasAssetMentionItems = (Array.isArray(data?.items) ? data.items : [])
            .filter(item => item?.url && (item.kind || 'image') === 'image')
            .filter(item => {
                const c = (data?.canvases || []).find(cv => cv.id === item.canvas_id);
                return !projectId || !c || (c.project || 'default') === projectId;
            })
            .map((item, index) => ({
                url:item.url,
                kind:'image',
                name:item.name || `项目图片${index + 1}`,
                alias:item.name || `项目图片${index + 1}`,
                assetId:item.id || '',
                role:'canvas-asset',
                canvasId:item.canvas_id || '',
                nodeId:item.node_id || '',
                imageIndex:Number.isFinite(Number(item.image_index)) ? Number(item.image_index) : '',
                natural_w:item.natural_w || item.width || '',
                natural_h:item.natural_h || item.height || '',
                mentionId:`canvas_asset_${index}_${Math.random().toString(36).slice(2, 7)}`
            }));
        canvasAssetMentionLoadedAt = now;
    } catch(_) {
        canvasAssetMentionItems = [];
        canvasAssetMentionLoadedAt = now;
    }
    return canvasAssetMentionItems;
}
function canvasAssetMentionCandidateImages(){
    return canvasAssetMentionItems || [];
}
function mentionCandidateImages(node, source=mentionSource){
    ...
    if(source === 'project') return canvasAssetMentionCandidateImages();
...
function mentionInputElement(){
    return activeMentionInputEl && document.body.contains(activeMentionInputEl) ? activeMentionInputEl : promptInput;
}
function mentionNodeForInput(){
    const input = mentionInputElement();
    const promptNodeId = input?.dataset?.promptNodeId || '';
    if(promptNodeId) return nodes.find(n => n.id === promptNodeId) || selectedNode();
    const posterFrameId = input?.dataset?.posterFrameId || '';
    if(posterFrameId) return nodes.find(n => n.id === posterFrameId) || selectedNode();
    return selectedNode();
}
...
function renderMentionPicker(source){
    const node = mentionNodeForInput();
    const inputItems = inputMentionCandidateImages(node);
    if(mentionInsertMode !== 'token' && !canvasAssetMentionLoadedAt){
        ensureCanvasAssetMentionItems().then(() => {
            const input = mentionInputElement();
            const shouldReopen = mentionInsertMode === 'token' && document.activeElement === input && /@$/.test(textBeforeCaret());
            if(mentionPicker?.classList?.contains('open') || shouldReopen) renderMentionPicker(canvasAssetMentionCandidateImages().length ? 'project' : (mentionSource === 'project' ? 'project' : source));
        });
    }
    const tokenMode = mentionInsertMode === 'token';
    const projectItems = tokenMode ? [] : canvasAssetMentionCandidateImages();
    ...
    const hasAssets = !tokenMode && Boolean(libraryWithMentionAssets);
    mentionSource = tokenMode ? 'input' : (source || (hasProject ? 'project' : (hasInput ? 'input' : 'asset')));
    ...
    if(mentionSource === 'project' && !hasProject) mentionSource = hasInput ? 'input' : 'asset';
    if(mentionSource === 'asset' && !hasAssets && hasInput) mentionSource = 'input';
    if(mentionSource === 'asset' && !hasAssets && hasProject) mentionSource = 'project';
    if(mentionSource === 'input' && !hasInput && hasProject) mentionSource = 'project';
    if(!hasInput && !hasAssets && !hasProject){ closeMentionPicker(); return; }
    ...
    const candidates = mentionCandidateImages(node, mentionSource).slice(0, 36);
    ...
    mentionPicker.innerHTML = `
        <div class="mention-picker-shell">
            <div class="mention-source-tabs">
                ...
                <button class="mention-source-tab ${mentionSource === 'project' ? 'active' : ''}" type="button" data-mention-source="project" title="项目图片" ${hasProject ? '' : 'disabled'}>
                    <i data-lucide="image"></i><span>项目图片</span>
                </button>
    ...
    if(mentionInsertMode === 'manual-ref'){
        ...
        placeMentionPickerForActiveInput();
    }
...
function showMentionPicker(){
    ...
    placeMentionPickerForActiveInput();
    mentionSource = 'input';
    if(!canvasAssetMentionLoadedAt){
        ensureCanvasAssetMentionItems().then(() => {
            const input = mentionInputElement();
            if(document.activeElement === input && /@$/.test(textBeforeCaret())) renderMentionPicker('input');
        });
        return;
    }
...
function placeMentionPickerForActiveInput(){
    const input = mentionInputElement();
    if(input === promptInput) return placeMentionPickerInPromptRow();
    const card = input?.closest?.('.prompt-node-card,.poster-frame-card');
...
function positionMentionPickerAtCaret(){
    ...
    if(input !== promptInput){
        const card = input?.closest?.('.prompt-node-card,.poster-frame-card');
        if(!card) return;
        mentionPicker.style.left = '8px';
        const top = (input.offsetTop || 0) + (input.offsetHeight || 80) + 4;
...
function insertMentionToken(img){
    ...
    token.dataset.assetId = img.assetId || img.asset_id || img.id || '';
    ...
    if(input === promptInput){
        ...
        const node = mentionNodeForInput();
        if(node?.type === 'smart-frame') syncPosterFramePromptFromElement(node, input);
        else syncPromptNodeTextFromElement(node, input);
        const host = input.closest?.('.image-node');
        if(host && node?.type === 'smart-prompt') refreshPromptNodeSegmentsUi(host, node);
    }
    promptMentionUndoArmed = true;
    scheduleSave();
}
function collectPromptParts(root=promptInput){
    const parts = [];
    const walk = node => {
        ...
        if(node.classList?.contains('mention-image-token')){
            ...
            parts.push({type:'image', kind, url:node.dataset.url || '', name:node.dataset.name || (kind === 'audio' ? '音频' : '图片'), assetId:node.dataset.assetId || '', nodeId:node.dataset.nodeId || '', imageIndex:Number(node.dataset.imageIndex || 0), asset_uris:assetUris});
            return;
        }
        if(node.tagName === 'BR'){
...
function posterFrameRunSettings(node){
    return posterFrameNormalizedRunSettings(smartSettingsForNode(node));
}
function posterFrameMeta(node, prompt, refs=[], runSettings=null){
    return {
        prompt,
        ...
        settings:cloneSmartSettings(runSettings || posterFrameRunSettings(node)),
        createdAt:Date.now()
    };
}
function finalizePosterFrameNode(node, outputs, meta, kind='image'){
    const live = liveSmartNode(node);
    if(!live) return;
    const ext = kind === 'video' ? 'mp4' : 'png';
    const additions = cleanHistoryImages((resultMediaUrls(outputs) || []).map((item, i) => {
        const url = typeof item === 'string' ? item : item?.url || '';
        return stripImageGenerationMeta(copyMediaSizeFields(item, {
            url,
            name:(typeof item === 'object' && item.name) || `poster-${i + 1}.${ext}`,
            kind:(typeof item === 'object' && item.kind) || kind,
            generatedResult:true
        }));
    }).filter(item => item.url));
    live.images = additions;
    live.outputKind = kind;
    live.pending = 0;
    live.running = false;
    delete live.pendingTasks;
    delete live.jimengPending;
    delete live.lastError;
    live.runFinishedAt = nowMs();
    if(!live.runStartedAt) live.runStartedAt = meta?.createdAt || live.runFinishedAt;
    live.runElapsedMs = Math.max(0, live.runFinishedAt - Number(live.runStartedAt || live.runFinishedAt));
    live.runTimerHidden = false;
    attachRunMeta(live, meta);
}
async function resumePosterFramePendingNode(node, logContext={}){
    const tasks = smartPendingTasks(node);
    if(!tasks.length) return;
    node.pending = Math.max(1, tasks.length);
    node.running = false;
    render();
    const outputs = [];
    const failures = [];
    await Promise.all(tasks.map(async task => {
        try {
            ...
            outputs.push(...resultMediaUrls(result?.image_items?.length ? result.image_items : (result?.images?.length ? result.images : result)));
        } catch(e) {
            failures.push(e);
        }
    }));
    if(outputs.length){
        finalizePosterFrameNode(node, outputs, logContext?.meta || posterFrameMeta(node, node.prompt || '', posterFrameReferenceImages(node), node.runSettings), 'image');
        addSmartGenerationLog({run:logContext.run, outputs:node.images || [], runMs:nowMs() - Number(logContext.runLogStart || nowMs())});
        render();
        scheduleSave();
        return;
    }
    throw failures[0] || new Error(tr('smart.errRunFailed'));
}
async function runPosterFrameNode(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    if(!isPosterFrameNode(node)) return;
    const prompt = String(node.prompt || '').trim();
    if(!prompt){ toast('请先填写画框提示词'); return; }
    if(smartNodeInFlight(node)) return;
    const previousSettings = cloneSmartSettings(settings);
    const runSettings = posterFrameRunSettings(node);
    settings = {...settings, ...cloneSmartSettings(runSettings)};
    const refs = posterFrameReferenceImages(node);
    const meta = posterFrameMeta(node, prompt, refs, runSettings);
    const runLog = smartRunSnapshot(node, prompt, refs, 'image');
    const runLogStart = nowMs();
    pushUndo();
    node.pending = 1;
    node.running = true;
    ...
    try {
        let outputs = [];
        if(settings.engine === 'comfy'){
            if((settings.comfyMode || 'text') !== 'text') throw new Error('画框节点暂只支持 ComfyUI 文生图模式');
            const urls = await runComfyText(node, prompt, node, meta);
            outputs = resultMediaUrls(urls || node.images || []);
        } else if(settings.engine === 'runninghub' && !runningHubSelectedModel(settings)){
            outputs = resultMediaUrls(await runRunningHubGeneration(prompt, refs, runSettings));
        } else if(settings.engine === 'modelscope'){
            outputs = resultMediaUrls(await runModelscopeGeneration(prompt, refs, runSettings));
        } else {
            const rhModelMode = settings.engine === 'runninghub' && Boolean(runningHubSelectedModel(settings));
            const taskResult = rhModelMode
                ? await runApiGeneration(prompt, refs, runningHubModelApiSettings(settings))
                : await runApiGeneration(prompt, refs, runSettings);
            const taskIds = Array.isArray(taskResult?.taskIds) ? taskResult.taskIds : [];
            if(!taskIds.length) throw new Error(tr('smart.errRunFailed'));
            node.pendingTasks = taskIds.map(taskId => ({taskId, kind:'image', providerId:taskResult.providerId, model:taskResult.model}));
            node.pending = Math.max(taskIds.length, 1);
            node.running = false;
            render();
            scheduleSave();
            await saveCanvas();
            await resumePosterFramePendingNode(node, {run:runLog, runLogStart, meta});
            settings = previousSettings;
            return;
        }
        if(!outputs.length) throw new Error(tr('smart.errNoOutImages'));
        finalizePosterFrameNode(node, outputs, meta, 'image');
        addSmartGenerationLog({run:runLog, outputs:node.images || [], runMs:nowMs() - runLogStart});
        settings = previousSettings;
        render();
        scheduleSave();
    } catch(e) {
        settings = previousSettings;
        if(handleJimengPendingSignal(node, e)) return;
        node.pending = 0;
        node.running = false;
        delete node.pendingTasks;
        node.lastError = (e.message || tr('smart.errRunFailed')).slice(0, 180);
        addSmartGenerationLog({run:runLog, outputs:[], runMs:nowMs() - runLogStart, error:node.lastError});
        toast(node.lastError);
        render();
        scheduleSave();
    }
}
async function optimizePosterFramePrompt(frameId){
    const node = nodes.find(n => n.id === frameId);
    if(!isPosterFrameNode(node)) return;
    const promptEl = document.querySelector(`.poster-frame-prompt[data-poster-frame-id="${CSS.escape(frameId)}"]`);
    let prompt = (promptEl?.textContent || node.prompt || '').trim();
    if(!prompt){ toast('请先填写提示词'); return; }
    const runSettings = posterFrameRunSettings(node);
    const provider = runSettings.provider_id || '';
    const model = runSettings.model || '';
    if(!provider){ toast('请先选择接口平台'); return; }
    const btn = document.querySelector(`[data-poster-frame-optimize="${CSS.escape(frameId)}"]`);
    if(!btn) return;
    const isRunning = btn.disabled;
    if(isRunning) return;
    btn.disabled = true;
    const originalHtml = btn.innerHTML;
    btn.innerHTML = '<i data-lucide="loader-2"></i><span>优化中</span>';
    lucide.createIcons();
    try {
        ...
        if(!res.ok){
            let detail = '';
            try { const err = await res.json(); detail = err.detail || ''; } catch(e){}
            throw new Error(detail || `优化失败 (${res.status})`);
        }
        const data = await res.json();
        const optimized = (data.optimized_prompt || '').trim();
        if(!optimized) throw new Error('AI 未返回优化结果');
        if(promptEl) promptEl.textContent = optimized;
...
async function runApiVideoGeneration(prompt, refs, runSettings=settings){
    ...
    try {
        ...
        const payload = {
            ...
            duration: normalizeSmartVideoDuration(runSettings.videoDuration, runSettings.videoProvider),
...
function splitPosterFrameInput(text){
    const raw = String(text || '').trim();
    if(!raw) return [];
    const normalized = raw.replace(/\r\n?/g, '\n');
    const sectionMarker = /^\s*(?:第\s*(?:\d+|[一二三四五六七八九十百]+)\s*(?:组|张|个|套|款)|(?:海报|画框|提示词)\s*(?:\d+|[一二三四五六七八九十百]+))\s*[：:、.)）-]/;
    const sectionLines = normalized.split('\n');
    const sections = [];
    let current = [];
    sectionLines.forEach(line => {
        if(sectionMarker.test(line) && current.length){
    ...
    const sectionItems = sections.filter(Boolean);
    if(sectionItems.length > 1) {
        return sectionItems.map((prompt, index) => {
            const first = prompt.split('\n').map(s => s.trim()).find(Boolean) || '';
            const title = first.replace(sectionMarker, '').trim().slice(0, 32) || `产品海报 ${index + 1}`;
            return {title, prompt};
        });
    }
    const numbered = normalized
        .split(/\n(?=\s*(?:\d+|[一二三四五六七八九十]+)[\.、)：:)])/)
        .map(s => s.replace(/^\s*(?:\d+|[一二三四五六七八九十]+)[\.、)：:)]\s*/, '').trim())
        .filter(Boolean);
    if(numbered.length > 1) return numbered.map((prompt, index) => ({title:`产品海报 ${index + 1}`, prompt}));
    const blank = normalized.split(/\n{2,}/).map(s => s.trim()).filter(Boolean);
    if(blank.length > 1) return blank.map((prompt, index) => ({title:`产品海报 ${index + 1}`, prompt}));
    const lines = normalized.split('\n').map(s => s.trim()).filter(Boolean);
    const substantialLines = lines.filter(line => line.length >= 36 && !/^(比例|画幅|风格|产品|内容|构图|合规|适用场景|场景|卖点|文案|背景|色调|材质|光影|镜头|Aspect Ratio|Style|Product|Content|Composition)\s*[：:]/i.test(line));
    if(lines.length > 1 && substantialLines.length === lines.length) {
        return lines.map((prompt, index) => ({title:`产品海报 ${index + 1}`, prompt}));
    }
    return [{title:'产品海报 1', prompt:normalized}];
}
function parsePosterFrameItemsFromText(text){
    const value = String(text || '').trim();
    if(!value) return [];
    const jsonMatch = value.match(/```(?:json)?\s*([\s\S]*?)```/i);
    const candidates = [value, jsonMatch?.[1] || ''];
    for(const candidate of candidates){
        if(!candidate) continue;
        try {
            const parsed = JSON.parse(candidate);
            const arr = Array.isArray(parsed) ? parsed : (Array.isArray(parsed?.frames) ? parsed.frames : []);
            const items = arr.map((item, index) => ({
                title:String(item?.title || item?.name || `产品海报 ${index + 1}`).trim(),
                prompt:String(item?.prompt || item?.positive || item?.text || '').trim(),
                sizeHint:String(item?.size || item?.ratio || item?.aspect_ratio || '').trim()
            })).filter(item => item.prompt);
            if(items.length) return items;
        } catch(_) {}
    }
    return splitPosterFrameInput(value);
}
function parsePosterFrameJsonItemsOnly(text){
    const value = String(text || '').trim();
    if(!value) return [];
    const jsonMatch = value.match(/```(?:json)?\s*([\s\S]*?)```/i);
    const candidates = [value, jsonMatch?.[1] || ''];
...
function posterFrameAiSplitSystemPrompt(){
    return '你是商业产品海报提示词拆分器。你必须只输出严格 JSON，不输出解释，不输出 Markdown。';
}
function posterFrameAiSplitUserPrompt(rawText){
    return [
        '任务：把用户粘贴的批量产品海报需求，整理成若干张“完整可生图的产品海报提示词卡”。',
        '',
        '输出格式：只返回 JSON 数组，格式如下：',
        '[{"title":"产品海报 1","prompt":"完整生图提示词","size":"建议画幅，如 1:1、4:5、9:16、4:3"}]',
        '',
        '拆分规则：',
        '1. 先判断用户实际要做多少张海报。只有出现明确的“第X张/第X组/海报X/方案X/产品海报X/1./2.”等完整海报边界，才拆成多张。',
        '2. 不要把同一张海报里的字段拆开。视觉主题、背景、产品组合、标题文案、底部小字、风格、构图、合规说明，必须合并到同一张 prompt 里。',
        '3. 如果用户只是给了一份完整海报方案，即使有很多段落，也只能输出 1 张提示词卡。',
        '4. 如果用户明确说要 10 张，就输出 10 条；如果说写多少张就多少张，按明确海报边界数量输出。',
        '5. 每条 prompt 必须是一张完整商业产品海报生图提示词，包含：主体产品、画面场景、背景、构图、文字排版区域、质感、光影、风格、限制项。',
        '6. 不要生成步骤卡、分镜卡、镜头卡、字段卡；不要输出“比例”这种单独卡。',
        '7. title 用“产品海报 1/2/3”或用户原本的小标题。',
        '8. size 从用户文本推断；没有明确画幅时填“产品海报”。',
        '',
        '用户粘贴内容：',
        rawText
    ].join('\n');
}
async function buildPosterFrameItemsWithAI(rawText, sourceNode=null){
    const provider = posterFrameChatProviderId(sourceNode?.splitProvider || '');
    const model = resolveChatModel(sourceNode?.splitModel || '', provider);
    const message = posterFrameAiSplitUserPrompt(rawText);
    const result = await fetch('/api/canvas-llm', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({
            ...
            provider,
            ms_model: provider === 'modelscope' ? model : '',
            system_prompt:posterFrameAiSplitSystemPrompt()
        })
    }).then(async r => {
        if(!r.ok) throw new Error(await responseErrorMessage(r, '提示词生成失败'));
        return r.json();
    });
    const items = parsePosterFrameJsonItemsOnly(result.text || '');
    if(!items.length) throw new Error('AI 没有返回 JSON 数组，未创建提示词卡');
    if(sourceNode){
        sourceNode.splitProvider = provider;
        sourceNode.splitModel = model;
    }
    return items;
}
function createPosterFramesFromItems(items, sourceNode=null, options={}){
    const list = (items || []).filter(item => String(item?.prompt || '').trim());
    if(!list.length){ toast('没有可创建的提示词卡'); return []; }
    pushUndo();
    const inheritedRefs = sourceNode ? posterFrameReferenceImages(sourceNode) : [];
    if(sourceNode){
        const staleIds = new Set([
            ...(Array.isArray(sourceNode.createdFrameIds) ? sourceNode.createdFrameIds : []),
            ...nodes.filter(n => n?.sourceBatchNodeId === sourceNode.id).map(n => n.id)
        ].filter(Boolean));
        if(staleIds.size){
            nodes = nodes.filter(n => !staleIds.has(n.id));
            canvas.connections = (canvas.connections || []).filter(c => !staleIds.has(c.from) && !staleIds.has(c.to));
            nodes.forEach(n => {
                if(Array.isArray(n.inputNodeIds)) n.inputNodeIds = n.inputNodeIds.filter(id => !staleIds.has(id));
            });
        }
    }
    const groupId = options.groupId || (sourceNode ? smartGroupContainingNode(sourceNode.id)?.id : '') || '';
    const start = sourceNode
        ? {x:(Number(sourceNode.x) || 0) + (Number(sourceNode.w) || 420) + 110, y:Number(sourceNode.y) || 0}
        : (options.point || viewportCenter());
    ...
    list.forEach((item, index) => {
        const col = index % cols;
        const row = Math.floor(index / cols);
        const itemRatio = posterFrameRatioFromText(item.sizeHint || item.size || item.ratio || item.aspect_ratio || '');
        const itemRunSettings = itemRatio ? {...settings, ratio:itemRatio} : settings;
        const node = createPosterFrameNode(
            Math.round(start.x + col * gapX),
            Math.round(start.y + row * gapY),
            {
                title:item.title || `产品海报 ${index + 1}`,
                prompt:item.prompt,
                sizeHint:item.sizeHint || item.size || item.ratio || '产品海报',
                runSettings:itemRunSettings,
                select:false,
                skipUndo:true
            }
        );
        created.push(node);
        if(sourceNode) {
            connectInputNode(sourceNode.id, node.id);
            node.sourceBatchNodeId = sourceNode.id;
            if(inheritedRefs.length){
                node.manualInputRefs = inheritedRefs.map((img, refIndex) => ({
                    url:img.url,
                    name:img.name || `参考图${refIndex + 1}`,
                    kind:img.kind || mediaKindForItem(img),
                    nodeId:img.nodeId || '',
                    imageIndex:Number.isFinite(Number(img.imageIndex)) ? Number(img.imageIndex) : '',
                    asset_uris:img.asset_uris || {},
        ...
        const group = groupId ? nodes.find(n => n.id === groupId) : null;
        if(group) addNodeToSmartGroup(group, node);
    });
    if(sourceNode){
        sourceNode.createdFrameIds = created.map(n => n.id);
        sourceNode.splitOutput = JSON.stringify(list.map((item, index) => ({
            title:item.title || `产品海报 ${index + 1}`,
            prompt:item.prompt,
            size:item.sizeHint || item.size || item.ratio || '产品海报'
        })), null, 2);
        sourceNode.splitStatus = `已生成 ${created.length} 张提示词卡`;
    }
    selectedIds = created.map(n => n.id);
    selectedId = selectedIds.length === 1 ? selectedIds[0] : '';
    selectedImage = {nodeId:'', index:-1};
    render();
    scheduleSave();
    toast(`已创建 ${created.length} 张提示词卡`);
    return created;
}
async function createPosterFrameCardsFromBatch(nodeId, useAI=true){
    const node = nodes.find(n => n.id === nodeId);
    if(!isPosterFrameBatchNode(node)) return;
    const raw = String(node.batchText || '').trim();
    if(!raw){ toast('请先粘贴海报提示词'); return; }
    node.running = true;
    node.splitStatus = useAI ? '正在调用平台 AI 拆分提示词...' : '正在按文本直接拆分...';
    render();
    try {
        let items = [];
        if(useAI){
            items = await buildPosterFrameItemsWithAI(raw, node);
        } else {
            items = splitPosterFrameInput(raw);
        }
        if(!items.length){ node.splitStatus = '没有识别到可用条目。'; toast(node.splitStatus); return; }
        createPosterFramesFromItems(items, node);
    } catch(err) {
        node.splitStatus = (err.message || '创建提示词卡失败').slice(0, 180);
        toast(node.splitStatus);
    } finally {
        node.running = false;
        render();
        scheduleSave();
...
function createNodeFromMenu(type){
    ...
    else if(type === 'script2storyboard') created = window.createScriptStoryboardNode ? window.createScriptStoryboardNode(p.x - 215, p.y - 305) : createImageNodeAt(p);
    else if(type === 'shotAssetCollector') created = window.createShotAssetCollectorNode ? window.createShotAssetCollectorNode(p.x - 280, p.y - 325) : createImageNodeAt(p);
    else if(type === 'assetHub') created = window.createAssetHubNode ? window.createAssetHubNode(p.x - 210, p.y - 260) : createImageNodeAt(p);
...
function activateComposerMentionInput(){ activeMentionInputEl = promptInput; }
promptInput.addEventListener('input', () => { activateComposerMentionInput(); maybeOpenMentionPicker(); });
...
promptInput.addEventListener('keyup', () => { activateComposerMentionInput(); maybeOpenMentionPicker(); });
promptInput.addEventListener('mouseup', () => { activateComposerMentionInput(); saveMentionRange(); });
promptInput.addEventListener('focus', () => { activateComposerMentionInput(); saveMentionRange(); });
promptInput.addEventListener('keydown', event => {
    activateComposerMentionInput();
    const key = String(event.key || '').toLowerCase();
    if((event.ctrlKey || event.metaKey) && key === 'z' && promptMentionUndoArmed && undoStack.length){
        event.preventDefault();
        promptMentionUndoArmed = false;
        performUndo();
        setTimeout(updateComposer, 0);
        return;
    }
    if(event.key === 'Escape') closeMentionPicker();
});
...
document.addEventListener('click', event => {
    ...
    if(!event.target.closest('.mention-picker') && !event.target.closest('#promptInput') && !event.target.closest('.prompt-node-text') && !event.target.closest('[data-input-add-reference]')) closeMentionPicker();
