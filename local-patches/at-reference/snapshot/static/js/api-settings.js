...
const DOUBAO_POOL_DEFAULT_BASE_URL = 'http://127.0.0.1:17888';
...
const DOUBAO_POOL_VIDEO_MODELS = ['seedance_2_0_mini', 'seedance_2_0_fast'];
...
const API_PROTOCOLS = ['openai', 'apimart', 'gemini', 'volcengine', 'runninghub', 'jimeng', 'doubao-pool', 'codex', 'gemini-cli'];
...
function updateIdPreview(){
    const item = provider();
    if(!item) return;
    const isBuiltin = item.id === 'comfly' || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'jimeng' || item.id === 'doubao-video';
...
function applyProviderOnboardingDefaults(id){
    ...
    if(id === 'modelscope'){
        ...
        item.image_models = unique([...(item.image_models || []).filter(model => !JIMENG_LEGACY_IMAGE_MODELS.has(String(model || '').trim())), ...JIMENG_DEFAULT_IMAGE_MODELS]);
        item.video_models = unique([...(item.video_models || []).filter(model => !JIMENG_LEGACY_VIDEO_MODELS.has(String(model || '').trim())), ...JIMENG_DEFAULT_VIDEO_MODELS]);
    } else if(id === 'doubao-video'){
        item.base_url = item.base_url || DOUBAO_POOL_DEFAULT_BASE_URL;
        item.protocol = 'doubao-pool';
        item.image_models = [];
        item.chat_models = [];
        item.video_models = unique((item.video_models || []).length ? item.video_models : DOUBAO_POOL_VIDEO_MODELS);
    } else if(id === 'codex'){
...
function syncEditor(){
    ...
    item.image_request_mode = normalizeImageRequestMode(
        item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'doubao-video' || CLI_PROTOCOLS.has(selectedProtocol)
            ? 'openai'
            : lockedApi
...
function updateProtocolFromInput(){
    const item = provider();
    if(!item || !protocolInput || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'doubao-video') return;
...
function renderEditor(){
    ...
    if(protocolInput){
        protocolInput.value = item.id === 'runninghub' ? 'runninghub' : item.id === 'volcengine' ? 'volcengine' : item.id === 'doubao-video' ? 'doubao-pool' : (item.protocol || 'openai');
    ...
    if(imageRequestModeInput){
        ...
        imageRequestModeInput.disabled = Boolean(lockedApi) || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'doubao-video' || CLI_PROTOCOLS.has(String(protocolInput?.value || item.protocol || '').toLowerCase());
    ...
    if(imageEditRouteInput){
        ...
        imageEditRouteInput.disabled = item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'doubao-video' || CLI_PROTOCOLS.has(String(protocolInput?.value || item.protocol || '').toLowerCase());
    }
    keyInput.value = '';
...
async function probeAsync(){
    ...
    try {
        ...
        const keepManualProtocol = ['gemini', 'volcengine', 'jimeng', 'doubao-pool', 'codex', 'gemini-cli'].includes(currentProtocol);
        ...
        const keepManualProtocol = ['gemini', 'volcengine', 'jimeng', 'doubao-pool', 'codex', 'gemini-cli'].includes(String(protocolInput?.value || item.protocol || '').toLowerCase());
...
const FIXED_PROTOCOL_PROVIDER_IDS = new Set(['modelscope', 'volcengine', 'runninghub', 'doubao-video']);
...
async function saveProviders(){
    ...
    providers.forEach(item => {
        ...
        item.image_request_mode = normalizeImageRequestMode(
            item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'doubao-video' || isCliProtocol
                ? 'openai'
    ...
    try {
        const res = await fetch('/api/providers', {
            ...
            body:JSON.stringify(providers.map(item => ({
                ...
                protocol:(item.id === 'modelscope') ? 'openai' : item.id === 'runninghub' ? 'runninghub' : item.id === 'volcengine' ? 'volcengine' : item.id === 'doubao-video' ? 'doubao-pool' : (item.protocol || 'openai'),
