...
function sanitizeVideoNodeProviderModel(node){
    ...
    node.duration = normalizeVideoDurationForProvider(node.duration || 5, node.apiProvider);
}
...
function addScriptStoryboardNode(point){
    ...
    return addNode({
        ...
        llmProvider:providerId,
        model:resolveChatModel('', providerId),
        storyboardPrompt:window.ScriptToStoryboard?.DEFAULT_PROMPT || '',
        shots:[],
        running:false,
        lastAiError:''
    });
}
function storyboardFieldRows(fields){
    return fields.map(([label, value]) => `<div class="storyboard-field"><b>${escapeHtml(label)}</b><span>${escapeHtml(value || '-')}</span></div>`).join('');
}
function storyboardFramePrompt(node, frameIndex){
    const frame = node?.shot?.frames?.[frameIndex] || {};
    if(window.ScriptToStoryboard?.buildFramePrompt) return window.ScriptToStoryboard.buildFramePrompt(node.shot || {}, frame, frameIndex);
    return frame.prompt || frame.description || '';
}
function storyboardVideoPrompt(node){
    return window.ScriptToStoryboard?.videoPromptText?.(node?.shot || {}) || node?.shot?.videoPrompt?.text || node?.shot?.sourceText || '';
}
function syncCanvasStoryboardFramesToVideoPrompt(node){
    if(!node?.shot || !Array.isArray(node.shot.frames)) return;
    node.shot.frames.forEach((frame, index) => {
        if(frame.locked) return;
        frame.prompt = storyboardFramePrompt(node, index);
    });
}
function renderStoryboardShotCardBody(node){
    ...
    shot.videoPrompt = shot.videoPrompt && typeof shot.videoPrompt === 'object' ? shot.videoPrompt : {text:storyboardVideoPrompt(node), source:'auto'};
    const wrap = document.createElement('div');
    wrap.className = 'storyboard-shot-card-body';
    const headerHtml = (window.ScriptToStoryboard?.storyboardHeaderFields?.(shot) || []).map(([label, value]) => `<div class="storyboard-meta-item"><b>${escapeHtml(label)}</b><span>${escapeHtml(value || '-')}</span></div>`).join('');
    const visualExtractText = window.ScriptToStoryboard?.visualExtractText?.(shot) || '暂无提取信息';
    const videoPrompt = storyboardVideoPrompt(node);
    const framesHtml = shot.frames.map((frame, index) => `
        <div class="storyboard-frame ${frame.locked ? 'locked' : ''}" data-frame-index="${index}">
            <div class="storyboard-frame-top"><strong>${escapeHtml(frame.label || `故事板帧${index + 1}`)}</strong><span>${escapeHtml(frame.frameId || `${shot.shotNumber || '镜头'}-F${index + 1}`)}</span></div>
            ...
            <div class="storyboard-frame-actions">
                <button class="storyboard-frame-regen" type="button" data-frame-index="${index}" ${frame.locked ? 'disabled' : ''}>单独重新生成</button>
                <button class="storyboard-frame-lock" type="button" data-frame-index="${index}">${frame.locked ? '解锁' : '锁定'}</button>
                <button class="storyboard-frame-edit" type="button" data-frame-index="${index}" ${frame.locked ? 'disabled' : ''}>编辑描述</button>
                <button class="storyboard-frame-preview" type="button" data-frame-index="${index}">生成写实预演图</button>
            </div>
        </div>
    `).join('');
    wrap.innerHTML = `
        <div class="storyboard-card-label">镜头故事板卡</div>
        <div class="storyboard-shot-meta">${headerHtml}</div>
        <div class="storyboard-visual-extract">
            <div class="storyboard-visual-extract-head">
                <b>画面提取</b>
                <button class="storyboard-copy-extract" type="button">复制</button>
            </div>
        ...
        <div class="storyboard-video-prompt">
            <div class="storyboard-video-prompt-head">
                <b>视频提示词</b>
                <div>
                    <button class="storyboard-copy-video" type="button">复制</button>
                    <button class="storyboard-refresh-from-video" type="button">刷新故事板</button>
                </div>
            </div>
            <textarea class="storyboard-video-text">${escapeHtml(videoPrompt)}</textarea>
        </div>
        <div class="storyboard-shot-actions">
            <button class="storyboard-video-node" type="button">生成视频节点</button>
            <button class="storyboard-shot-preview" type="button">生成整镜故事板</button>
        </div>
        <div class="storyboard-frame-list">${framesHtml}</div>
    `;
    const videoText = wrap.querySelector('.storyboard-video-text');
    if(videoText){
        bindScrollableText(videoText);
        videoText.onmousedown = e => e.stopPropagation();
        videoText.onclick = e => e.stopPropagation();
        videoText.oninput = e => {
            node.shot = node.shot || {};
            ...
            syncCanvasStoryboardFramesToVideoPrompt(node);
            scheduleSave();
        };
    }
    wrap.querySelectorAll('.storyboard-copy-extract').forEach(btn => {
        btn.onclick = async e => {
            e.preventDefault();
            e.stopPropagation();
            const text = window.ScriptToStoryboard?.visualExtractText?.(node.shot || {}) || '暂无提取信息';
            if(!(await copyTextToClipboard(text))) alert(text);
            else toast('已复制画面提取');
        };
    });
    wrap.querySelectorAll('.storyboard-copy-video').forEach(btn => {
        btn.onclick = async e => {
            e.preventDefault();
            e.stopPropagation();
            if(!(await copyTextToClipboard(storyboardVideoPrompt(node)))) alert(storyboardVideoPrompt(node));
            else toast('已复制视频提示词');
        };
    });
    wrap.querySelectorAll('.storyboard-refresh-from-video').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            syncCanvasStoryboardFramesToVideoPrompt(node);
    ...
    wrap.querySelectorAll('[data-frame-desc]').forEach(ta => {
    ...
    wrap.querySelectorAll('.storyboard-frame-lock').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            const frame = node.shot?.frames?.[Number(btn.dataset.frameIndex) || 0];
            if(frame) frame.locked = !frame.locked;
            render();
            scheduleSave();
        };
    });
    wrap.querySelectorAll('.storyboard-frame-edit').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            const ta = wrap.querySelector(`[data-frame-desc="${Number(btn.dataset.frameIndex) || 0}"]`);
            if(ta && !ta.disabled) ta.focus();
        };
    });
    wrap.querySelectorAll('.storyboard-frame-regen').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            const index = Number(btn.dataset.frameIndex) || 0;
            const frame = node.shot?.frames?.[index];
            if(frame?.locked) return;
    ...
    wrap.querySelectorAll('.storyboard-frame-preview').forEach(btn => {
        btn.onclick = async e => {
            e.preventDefault();
            e.stopPropagation();
            createStoryboardPreviewGenerator(node, Number(btn.dataset.frameIndex) || 0);
        };
    });
    wrap.querySelectorAll('.storyboard-shot-preview').forEach(btn => {
        btn.onclick = async e => {
            e.preventDefault();
            e.stopPropagation();
            createStoryboardWholePreviewGenerator(node);
        };
    });
    wrap.querySelectorAll('.storyboard-video-node').forEach(btn => {
        btn.onclick = async e => {
            e.preventDefault();
            e.stopPropagation();
            createStoryboardVideoGenerator(node);
        };
    });
    return wrap;
}
function createStoryboardPreviewGenerator(card, frameIndex=0){
    if(!ensureCanvas()) return null;
    const prompt = storyboardFramePrompt(card, frameIndex);
    const baseX = Number(card.x || 0) + Number(card.w || defaultNodeSize('storyboardCard').w || 560) + 80;
    const baseY = Number(card.y || 0) + Math.max(0, frameIndex) * 120;
    const promptNode = addPromptNode({x:baseX, y:baseY});
    if(!promptNode) return null;
    promptNode.text = prompt;
    const gen = addGeneratorNode({x:baseX + 420, y:baseY});
    if(gen && !connections.some(c => c.from === promptNode.id && c.to === gen.id)){
        connections.push({id:uid('c'), from:promptNode.id, to:gen.id});
    }
    if(gen){
        storyboardReferenceNodesFor(card, prompt).forEach(refNode => {
            if(refNode.id === gen.id) return;
            if(!canConnect(refNode.id, gen.id)) return;
            if(!connections.some(c => c.from === refNode.id && c.to === gen.id)){
                connections.push({id:uid('c'), from:refNode.id, to:gen.id});
            }
        });
    }
    selected.clear();
    selected.add(gen?.id || promptNode.id);
    syncGeneratorInputs();
    render();
    scheduleSave();
    toast('已创建写实预演图生成节点');
    return gen || promptNode;
}
function storyboardWholePrompt(card){
    if(window.ScriptToStoryboard?.buildWholeStoryboardPrompt) return window.ScriptToStoryboard.buildWholeStoryboardPrompt(card?.shot || {});
...
function createStoryboardWholePreviewGenerator(card){
    if(!ensureCanvas()) return null;
    const prompt = storyboardWholePrompt(card);
    const offsetIndex = Math.max(0, card?.shot?.frames?.length || 0);
    const baseX = Number(card.x || 0) + Number(card.w || defaultNodeSize('storyboardCard').w || 560) + 80;
    const baseY = Number(card.y || 0) + offsetIndex * 120;
    const promptNode = addPromptNode({x:baseX, y:baseY});
    if(!promptNode) return null;
    promptNode.text = prompt;
    promptNode.name = `${card?.shot?.shotNumber || '镜头'} 整镜故事板`;
    const gen = addGeneratorNode({x:baseX + 420, y:baseY});
    if(gen && !connections.some(c => c.from === promptNode.id && c.to === gen.id)){
        connections.push({id:uid('c'), from:promptNode.id, to:gen.id});
    }
...
function createStoryboardVideoGenerator(card){
    if(!ensureCanvas()) return null;
    const prompt = storyboardVideoPrompt(card);
    const offsetIndex = Math.max(0, (card?.shot?.frames?.length || 0) + 1);
    const baseX = Number(card.x || 0) + Number(card.w || defaultNodeSize('storyboardCard').w || 560) + 80;
    const baseY = Number(card.y || 0) + offsetIndex * 120;
    const promptNode = addPromptNode({x:baseX, y:baseY});
    if(!promptNode) return null;
    promptNode.text = prompt;
    promptNode.name = `${card?.shot?.shotNumber || '镜头'} 视频提示词`;
    const video = addVideoNode({x:baseX + 420, y:baseY});
    if(video){
        video.duration = storyboardDurationSeconds(card?.shot || {});
        video.aspectRatio = video.aspectRatio || '16:9';
        if(!connections.some(c => c.from === promptNode.id && c.to === video.id)){
            connections.push({id:uid('c'), from:promptNode.id, to:video.id});
        }
        storyboardReferenceNodesFor(card, prompt).forEach(refNode => {
            if(refNode.id === video.id) return;
            if(!canConnect(refNode.id, video.id)) return;
            if(!connections.some(c => c.from === refNode.id && c.to === video.id)){
                connections.push({id:uid('c'), from:refNode.id, to:video.id});
            }
        });
    }
    selected.clear();
    selected.add(video?.id || promptNode.id);
    syncGeneratorInputs();
    render();
    scheduleSave();
    toast('已创建视频生成节点');
    return video || promptNode;
}
function upstreamStoryboardNodeIds(nodeId){
    const seen = new Set([nodeId]);
    const ordered = [];
    const walk = id => {
        (connections || []).filter(c => c.to === id).forEach(conn => {
            if(!conn?.from || seen.has(conn.from)) return;
            seen.add(conn.from);
            walk(conn.from);
            ordered.push(conn.from);
        });
    };
    walk(nodeId);
    return ordered;
}
function storyboardReferenceNodesFor(card, prompt=''){
    const ids = new Set();
    upstreamStoryboardNodeIds(card?.id).forEach(id => {
        const node = nodes.find(n => n.id === id);
        if(node && mediaRefsFromNode(node).length) ids.add(node.id);
    });
    canvasMentionReferenceSources(prompt, card?.id || '').forEach(source => {
        if(source.sourceNodeId) ids.add(source.sourceNodeId);
    });
    return [...ids].map(id => nodes.find(n => n.id === id)).filter(Boolean);
}
function renderStoryboardCardBody(node){
    if(node.cardKind === 'storyboard') return renderStoryboardShotCardBody(node);
    const shot = node.shot || {};
    ...
    const wrap = document.createElement('div');
    wrap.className = `storyboard-card-body ${node.cardKind || 'breakdown'}`;
    const actions = node.cardKind === 'visual'
        ? `<div class="storyboard-card-actions"><button class="storyboard-copy-all" type="button">复制全部信息</button><button class="storyboard-make-prompt" type="button">生成生图提示词</button></div>`
        : '';
    wrap.innerHTML = `<div class="storyboard-card-label">${node.cardKind === 'visual' ? '分镜画面卡' : '分镜拆解卡'}</div>${actions}${storyboardFieldRows(fields)}`;
    const copyBtn = wrap.querySelector('.storyboard-copy-all');
    ...
    const promptBtn = wrap.querySelector('.storyboard-make-prompt');
    if(promptBtn) promptBtn.onclick = e => {
        e.preventDefault();
        e.stopPropagation();
        createImagePromptCardFromVisual(node.id);
    };
    return wrap;
}
function imagePromptSourceVisualCard(card){
    if(!card) return null;
    const linked = connections
        .filter(c => c.to === card.id)
        .map(c => nodes.find(n => n.id === c.from))
        .find(n => n?.type === 'storyboardCard' && n.cardKind === 'visual');
    return linked || nodes.find(n => n.id === card.sourceVisualCardId && n.type === 'storyboardCard' && n.cardKind === 'visual') || null;
}
function syncImagePromptCardFromSource(card, options={}){
    if(!card || card.type !== 'imagePromptCard') return false;
    const source = imagePromptSourceVisualCard(card);
    if(!source?.shot) {
...
function renderImagePromptCardBody(node){
    syncImagePromptCardFromSource(node);
    const wrap = document.createElement('div');
    wrap.className = 'image-prompt-card-body';
    wrap.innerHTML = `
        <div class="storyboard-card-label">生图提示词卡</div>
        <div class="storyboard-prompt-head"><span>来源画面卡信息预览</span><button class="storyboard-sync-source" type="button">重新同步</button></div>
        <pre class="image-prompt-source">${escapeHtml(node.sourceInfo || '未连接分镜画面卡')}</pre>
        <label class="storyboard-label">生图提示词文本</label>
        <textarea class="image-prompt-text">${escapeHtml(node.promptText || '')}</textarea>
    `;
    const ta = wrap.querySelector('.image-prompt-text');
    bindScrollableText(ta);
    ta.oninput = e => {
    ...
    wrap.querySelector('.storyboard-sync-source').onclick = e => {
        e.preventDefault();
        e.stopPropagation();
        node.autoPrompt = true;
        syncImagePromptCardFromSource(node, {clearIfMissing:true});
        render();
        scheduleSave();
    };
    return wrap;
}
function renderStoryboardGroupBody(node){
    const wrap = document.createElement('div');
    wrap.className = 'storyboard-group-body';
    wrap.innerHTML = `
...
function renderScriptStoryboardBody(node){
    if(node.running && (!node.runStartedAt || Date.now() - Number(node.runStartedAt) > 15 * 60 * 1000)){
        node.running = false;
        node.runFinishedAt = Date.now();
        node.runElapsedMs = Math.max(0, node.runFinishedAt - Number(node.runStartedAt || node.runFinishedAt));
    }
    node.storyboardPrompt = node.storyboardPrompt || window.ScriptToStoryboard?.DEFAULT_PROMPT || '';
    node.llmProvider = resolveChatProviderId(node.llmProvider || '');
    node.model = resolveChatModel(node.model || '', node.llmProvider);
    const wrap = document.createElement('div');
    wrap.className = 'script-storyboard-body';
    wrap.innerHTML = `
        <label class="storyboard-label">完整分镜输入框</label>
        <textarea class="storyboard-script" placeholder="在这里粘贴已经写好的镜头分镜文本...">${escapeHtml(node.scriptText || '')}</textarea>
        <div class="storyboard-model-row">
            <select class="select-lite storyboard-provider">${chatProviderOptions(node.llmProvider)}</select>
            <select class="select-lite storyboard-model">${chatModelOptions(node.model, node.llmProvider)}</select>
        </div>
        <div class="storyboard-prompt-head">
            <span>内设提示词</span>
            <button class="secondary-btn storyboard-reset" type="button">恢复默认</button>
        </div>
        <textarea class="storyboard-prompt">${escapeHtml(node.storyboardPrompt)}</textarea>
        ${node.lastAiError ? `<div class="storyboard-warn">AI 调用失败，已使用本地拆分：${escapeHtml(node.lastAiError.slice(0, 80))}</div>` : ''}
        <button class="gen-btn storyboard-run ${node.running ? 'running' : ''}" type="button" ${node.running ? 'disabled' : ''}>
            <i data-lucide="sparkles" class="w-4 h-4"></i>${node.running ? '生成中...' : '生成故事板'}
        </button>
    `;
    const scriptEl = wrap.querySelector('.storyboard-script');
    const promptEl = wrap.querySelector('.storyboard-prompt');
    ...
    wrap.querySelector('.storyboard-provider').onchange = e => {
        node.llmProvider = resolveChatProviderId(e.target.value);
        node.model = resolveChatModel('', node.llmProvider);
        render();
        scheduleSave();
    };
    wrap.querySelector('.storyboard-model').onchange = e => {
        node.model = e.target.value;
        scheduleSave();
    };
    wrap.querySelector('.storyboard-reset').onclick = e => {
        e.preventDefault();
        e.stopPropagation();
        node.storyboardPrompt = window.ScriptToStoryboard?.DEFAULT_PROMPT || '';
        render();
        scheduleSave();
    };
    wrap.querySelector('.storyboard-run').onclick = e => {
        e.preventDefault();
        e.stopPropagation();
        runScriptStoryboardNode(node.id);
    };
    return wrap;
}
function addImagePromptCardNode(point, sourceVisualCard=null){
    const p = point || defaultPoint(130, 0);
    const node = {
        id:uid('s2p'),
        ...
        w:360,
        h:430,
        sourceStoryboardId:sourceVisualCard?.sourceStoryboardId || '',
        sourceVisualCardId:sourceVisualCard?.id || '',
        sourceInfo:'',
        promptText:'',
        autoPrompt:true
    };
    if(sourceVisualCard) syncImagePromptCardFromSource(node);
    return addNode(node);
}
function createImagePromptCardFromVisual(visualCardId){
    const source = nodes.find(n => n.id === visualCardId && n.type === 'storyboardCard' && n.cardKind === 'visual');
    if(!source) return null;
    const prompt = window.ScriptToStoryboard.imagePromptDraft(source.shot);
    copyTextToClipboard(prompt).then(ok => {
        if(ok) alert('已复制生图提示词');
        else alert(prompt);
    });
    return source;
}
function deleteStoryboardOutputs(sourceId){
...
function createStoryboardOutputs(source, shots){
    deleteStoryboardOutputs(source.id);
    ...
    shots.forEach((shot, index) => {
        ...
        nodes.push(card);
        made.push(card);
        connections.push({id:uid('c'), from:source.id, to:card.id});
    });
    return made;
}
async function runScriptStoryboardNode(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    if(!node || node.running) return;
    const script = String(node.scriptText || '').trim();
    if(!script){ alert('请先输入完整分镜文本'); return; }
    pushUndo();
    node.running = true;
    ...
    let shots = [];
    const localShots = window.ScriptToStoryboard.fallbackSplit(script);
    try {
        const helper = window.ScriptToStoryboard;
        const message = helper.buildMessage(script, node.storyboardPrompt);
        const text = await callCanvasLLM({
            id:node.id,
            type:'llm',
        ...
        shots = helper.normalizeShots(helper.extractJson(text));
        if(localShots.length > shots.length) shots = localShots;
        if(!shots.length) throw new Error('AI 未返回可解析镜头 JSON');
    } catch(err) {
        node.lastAiError = err.message || String(err);
        shots = localShots;
    } finally {
        node.running = false;
        node.runFinishedAt = Date.now();
        node.runElapsedMs = node.runFinishedAt - Number(node.runStartedAt || node.runFinishedAt);
    }
    try {
        node.shots = shots;
        createStoryboardOutputs(node, shots);
        selected.clear();
...
function render(){
    const legacyPromptIds = new Set(nodes.filter(n => n.type === 'imagePromptCard').map(n => n.id));
    if(legacyPromptIds.size){
        nodes = nodes.filter(n => !legacyPromptIds.has(n.id));
        connections = connections.filter(c => !legacyPromptIds.has(c.from) && !legacyPromptIds.has(c.to));
        scheduleSave();
    }
...
function renderNode(node){
    ...
    if(node.type === 'scriptStoryboard') body.appendChild(renderScriptStoryboardBody(node));
    if(node.type === 'storyboardGroup') body.appendChild(renderStoryboardGroupBody(node));
    if(node.type === 'storyboardCard') body.appendChild(renderStoryboardCardBody(node));
...
function defaultNodeSize(type){
    ...
    if(type === 'scriptStoryboard') return {w:430, h:610};
    if(type === 'storyboardGroup') return {w:230, h:132};
...
function renderVideoBody(node){
    ...
    const fixedDurationLocked = hasFixedVideoDuration(node.apiProvider);
    const durationControl = fixedDurationLocked
        ? `<select class="select-lite video-duration compact-select">
                <option value="5" ${Number(node.duration || 5) === 5 ? 'selected' : ''}>5 秒</option>
                <option value="10" ${Number(node.duration || 5) === 10 ? 'selected' : ''}>10 秒</option>
            </select>`
    ...
    const applyDurationChange = e => {
        e.stopPropagation();
        node.duration = normalizeVideoDurationForProvider(e.target.value || 5, node.apiProvider);
        e.target.value = String(node.duration);
        scheduleSave();
    };
    durationSelect.oninput = applyDurationChange;
    durationSelect.onchange = applyDurationChange;
    durationSelect.onblur = e => { e.target.value = String(normalizeVideoDurationForProvider(node.duration || 5, node.apiProvider)); };
...
function canvasMentionNames(text){
    const names = [];
    const seen = new Set();
...
function canvasMentionKey(value){
    return String(value || '').trim().toLowerCase().replace(/\.[a-z0-9]+$/i, '').replace(/\s+/g, '');
}
function canvasReferenceCandidates(){
    const result = [];
    nodes.forEach(node => {
        mediaRefsFromNode(node).forEach((ref, index) => {
            if(!ref?.url) return;
...
function canvasMentionReferenceSources(text, promptNodeId=''){
    const names = canvasMentionNames(text);
    if(!names.length) return [];
    const candidates = canvasReferenceCandidates();
    const seen = new Set();
    const out = [];
    names.forEach(name => {
        const key = canvasMentionKey(name);
        const hit = candidates.find(item => item.aliases.some(alias => canvasMentionKey(alias) === key));
        if(!hit?.ref?.url || seen.has(hit.ref.url)) return;
        seen.add(hit.ref.url);
        out.push({
            id:`mention:${promptNodeId}:${hit.node.id}:${hit.ref.url}`,
            type:'mentionImage',
            label:`@${name}`,
            preview:hit.ref.url,
            refs:[hit.ref],
...
function generatorSources(gen){
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        ...
        if(n.type === 'prompt') return [
            {id:n.id, type:'prompt', label:(n.text || '提示词').slice(0, 32), refs:[], prompt:n.text || ''},
            ...canvasMentionReferenceSources(n.text || '', n.id)
        ];
        if(n.type === 'loop') {
    ...
    try {
        const result = await cascadeFetch('/api/canvas-video', {
            ...
            body:JSON.stringify({
                ...
                duration:normalizeVideoDurationForProvider(node.duration || 5, node.apiProvider),
        ...
        if(target){
            ...
            if(canConnect(fromId, toId)){
                if(!connections.some(c => c.from === fromId && c.to === toId)){
                    ...
                    connections.push({id:uid('c'), from:fromId, to:toId});
                    syncLatestGeneratedOutputToConnection(fromId, toId);
                    const promptCard = nodes.find(n => n.id === toId && n.type === 'imagePromptCard');
                    if(promptCard) syncImagePromptCardFromSource(promptCard, {clearIfMissing:true});
                }
                syncGeneratorInputs();
                scheduleSave();
                render();
...
function canConnect(fromId, toId){
    ...
    if(from.type === 'scriptStoryboard') return to.type === 'storyboardGroup' || to.type === 'storyboardCard';
    if(to.type === 'scriptStoryboard') return ['image','group','output', ...CANVAS_MEDIA_OUTPUT_TYPES].includes(from.type);
    if(from.type === 'storyboardGroup') return to.type === 'storyboardCard';
    if(from.type === 'storyboardCard' && from.cardKind === 'visual') return to.type === 'imagePromptCard';
    if(['storyboardGroup','storyboardCard'].includes(to.type)) return ['scriptStoryboard','storyboardGroup'].includes(from.type);
    if(to.type === 'imagePromptCard') return from.type === 'storyboardCard' && from.cardKind === 'visual';
